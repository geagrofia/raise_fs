...
# results_env <- new.env()
# 
# results_env$test_results_dollar <- paste0("$$ Testing that variables are assigned to a specific environment")

# # testing results_env assign
# AAA_test_results <-
#   function() {
#     paste0("Function -Testing that variables are assigned to a specific environment")
#   }
# 
# AAA_test_results_assign <-
#   function() {
#     test_results <-  paste0("Function -Testing that variables are assigned to a specific environment")
#     assign("test_results", test_results, envir = results_env)
#   }
# 
# AAA_test_results_assign_arrows <-
#   function() {
#     test_results <-  paste0("Function -Testing that variables are assigned to a specific environment")
#     results_env$test_results2 <- test_results
#   }
...
fn_create_rb <- function(lower_conc_values, weights, rb_conc_name) {
  ...
  # check the the higher level rule base conc levels are available
  
  cat("\n\n\n")
  cat("\n\nconc_levels_i1_1 Global\n")
  #print(get(paste("conc_levels_i1_1"), envir = results_env))
  print(get(paste("conc_levels_i1_1"), .GlobalEnv))
  #print(conc_levels_i1_1)
  
  cat("\n\nconc_levels_i1_2 Global\n")
  #print(get(paste("conc_levels_i1_2"), envir = results_env))
  print(get(paste("conc_levels_i1_2"), .GlobalEnv))
  #print(conc_levels_i1_2)
  
  cat("\n\nconc_levels_i1_3 Global\n")
  #print(get(paste("conc_levels_i1_3"), envir = results_env))
  print(get(paste("conc_levels_i1_3"), .GlobalEnv))
  #print(conc_levels_i1_3)
  ...
  if (any(grepl("good",lower_conc_values))) {
    ...
    # conc_options <- c(get(paste("conc_levels_i1_1"), .GlobalEnv),
    #                   get(paste("conc_levels_i1_2"), .GlobalEnv),
    #                   get(paste("conc_levels_i1_3"), .GlobalEnv))
    
    conc_options <- c(conc_levels_i1_1, conc_levels_i1_2, conc_levels_i1_3)
    
    
    c_o <- "sef"
  }
  
  if (any(grepl("high",lower_conc_values))) {
    #conc_options <- c("poor", "moderate", "good")
    # conc_options <- c(get(paste("conc_levels_i1_1"), .GlobalEnv),
    #                   get(paste("conc_levels_i1_2"), .GlobalEnv),
    #                   get(paste("conc_levels_i1_3"), .GlobalEnv))
...
fn_fill_fp_list <- function(df_one_many) {
  ...
  if (nrow(df_one_many_data) > 0) {
    fp_prefix <- "fp_"
    df_one_many_data$fp <-
      paste(fp_prefix, df_one_many_data$crit_code.y, sep = "")
    #str(df_one_many_data)
    fp_list <-
      append(fp_list, list(
        paste(
          df_one_many_data$crit_code.y,
          " = ",
          df_one_many_data$fp
        )
      ))
  ...
  if (nrow(df_one_many_nodata) > 0) {
    for (i in 1:nrow(df_one_many_nodata)){
      fp_list <-
        append(fp_list, get(
          paste0("fp_",
                 df_one_many_nodata$crit_code.y[i],
                 "_list")
        ))
    }}
...
fn_fill_rb_list <- function(df_one_many) {
  ...
  if (nrow(df_one_many_data) > 0) {
    # rb_prefix <- "rb_"
    # df_one_many_data$rb <-
    #   paste(rb_prefix, df_one_many_data$crit_code.y, sep = "")
    # str(df_one_many_data)
    rb_list <-
      append(rb_list, list(
...
fn_rbs_names <- function(df_one_many) {
  ...
  if ("mapping_code" %in% colnames(df_one_many)) {
    df_one_many <- select(df_one_many, crit_code.y, mapping_code)
  } else {
    df_one_many <- select(df_one_many, crit_code.y)
    df_one_many$mapping_code <- df_one_many$crit_code.y
  }
...
classify_maps_FAO <-
  function(n,
    ...
    #           rast_clas_fao_cat, envir = results_env)
    
    assign(paste0("rast_clas_fao_cat", fpm_plot_title),
           rast_clas_fao_cat, .GlobalEnv) 
    
    
 
    # COMMENTED OUT 08/08/2025
    
    # list_rast_clas_FAO_cat <<-
    #   append(list_rast_clas_FAO_cat,
    #          paste0("rast_clas_fao_cat", fpm_plot_title))
    
    # END COMMENTED OUT 08/08/2025     
     
     # ADDED 08/08/2025  
     
     list_rast_clas_FAO_cat[[fpm_plot_title]] <<- rast_clas_fao_cat
     
     # END ADDED 08/08/2025  
     
     
     
#    assign("list_rast_clas_FAO_cat", 
#           append(
#             get(list_rast_clas_FAO_cat, envir = results_env),
#             get(paste0("rast_clas_fao_cat", fpm_plot_title), envir = results_env)), 
#           envir = results_env)
...
classify_maps_limits <-
  function(n,
    ...
    assign(paste0("rast_limits_max_FAO_", fpm_plot_title), rast_limits_max_FAO3, .GlobalEnv)
    ...
    list_rast_limits_max_FAO[[fpm_plot_title]] <<- rast_limits_max_FAO3
