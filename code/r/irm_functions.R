  ...
  function(data_file_prefix,
    ...
    raster_filename <-
      as.character(paste("spatial_data/input/",
                         ...
                         ".tif",
                         sep = ""))
    raster_name <-  raster_or_brick(here(raster_filename))
    return(raster_name)
  }
...
# original
#.growth_period <-function (day_begin, day_end) 
#{
#  days <- 1:365
...
rasterize_plot_fpm <- function(n, fpm_conc_var, fpm_conc_name, df_data) {
  ...
  dB <- rasterize(d0, ra_mask, fpm_conc_var[[1]]) %>% brick
  
  # in a loop add subsequent layers to the dB rasterbrick starting from the second layer (actually the addlayer changes dB to a rasterstack)
  
  for (i in 2:n) {
    dB <-
      addLayer(dB, rasterize(d0, ra_mask, field = fpm_conc_var[[i]]))
  }
  ...
  dB %>% plot(title = fpm_plot_title, n = 10)
}
...
#        fpm_plot_title is the title of the plot
#        df_data is the name of the data frame or tibble used to store the data
#        fpm_plot_title is the name of the plot



rasterize_plot_fpm_new <- function(n, fpm_conc_var, fpm_conc_name, df_data, fpm_plot_title) {
  d0 <- df_data %>%
    dplyr::select(unlist(as.character(noquote(fpm_conc_var)))) %>%
    na.omit %>%
    st_as_sf(coords = c("x", "y"))
