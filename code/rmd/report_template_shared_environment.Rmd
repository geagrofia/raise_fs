...
```{r , results='asis'}
# Now use the variables/functions from helpers.R
...
if (params$INT == 1)
  shiny::setProgress(0.02, message = "Loading requirements")  # set progress to 17%
```
...
# Example: create some dummy rasters (you can skip this if you already have a list)
...
raster_list <- list(r1, r2)  # your list of SpatRaster objects

# Loop and plot
for (i in seq_along(raster_list)) {
  plot(raster_list[[i]], main = paste("Raster", i))
}

```


```{r TEST_i1_reclass_soil_texture, results='asis'}
...
cat("soil_texture_filename_i1")
print(soil_texture_filename_i1)
cat("print(file.exists(soil_texture_filename_i1))")
print(file.exists(soil_texture_filename_i1))

if (file.exists(soil_texture_filename_i1)) {
  
  cat("EXISTS")
  
  
  df_soil_texture_i1 <- read.csv(soil_texture_filename_i1)
  
  cat("print(file.exists(df_soil_texture_i1))")
  print(df_soil_texture_i1)
  
  df_soil_texture_i1 %>%
    kable(digits = 3, caption = "Soil Texture ") %>% kable_styling("striped", full_width = T) 
...
if (params$INT == 1)
  shiny::setProgress(0.07, message = "Loading spatial data - vector")
```
...
# load farmers triangulation

vect_triangulation_farmers_i1 <- load_vector_data(paste0("triangulation_farmers_", params$DIVCODEVAL,"_", params$INN1))



vect_triangulation_farmers_i1$ID <- seq.int(nrow(vect_triangulation_farmers_i1))# add an id field
vect_triangulation_farmers_i1$id <- formatC(vect_triangulation_farmers_i1$ID, width = 2, format = "d", flag = "0") #format the id field


vect_triangulation_farmers_extent_i1 <- ext(vect_triangulation_farmers_i1) # get the extent
nudge_xvaltri_farmers_i1 <-
  ((vect_triangulation_farmers_extent_i1[2] - vect_triangulation_farmers_extent_i1[1]) /  (nrow(vect_triangulation_farmers_i1) * 2.5))
#cat("nudge_xvaltri =", nudge_xvaltri)
nudge_yvaltri_farmers_i1 <-
  ((vect_triangulation_farmers_extent_i1[4] - vect_triangulation_farmers_extent_i1[3]) /  (nrow(vect_triangulation_farmers_i1) * 2.5))
...
<div class="fold o">   
```{r i1_spatialdataload_vector_03, cache = FALSE, out.width="100%", include = TRUE, echo=FALSE}

# load agents triangulation

vect_triangulation_agents_i1 <- load_vector_data(paste0("triangulation_agents_", params$DIVCODEVAL,"_", params$INN1))

vect_triangulation_agents_i1$ID <- seq.int(nrow(vect_triangulation_agents_i1))# add an id field
vect_triangulation_agents_i1$id <- formatC(vect_triangulation_agents_i1$ID, width = 2, format = "d", flag = "0") #format the id field

vect_triangulation_agents_extent_i1 <- ext(vect_triangulation_agents_i1) # get the extent
nudge_xvaltri_agents_i1 <-
  ((vect_triangulation_agents_extent_i1[2] - vect_triangulation_agents_extent_i1[1]) /  (nrow(vect_triangulation_agents_i1) * 2.5))
#cat("nudge_xvaltri =", nudge_xvaltri)
nudge_yvaltri_agents_i1 <-
  ((vect_triangulation_agents_extent_i1[4] - vect_triangulation_agents_extent_i1[3]) /  (nrow(vect_triangulation_agents_i1) * 2.5))
#cat("nudge_yvaltri =", nudge_yvaltri)

if (params$INT == 1)
  shiny::setProgress(0.08, message = "Loading spatial data - raster")
```
</div>
...
if (params$INT == 1)
  shiny::setProgress(0.1, message = "Resampling spatial data")  # set progress to 10%
```
...
if (params$INT == 1)
  shiny::setProgress(0.15, message = "Converting raw data to criteria values")  # set progress to 17%
``` 
</div>
...
df_growth_stages_i1 %>% dplyr::filter(gs_name != "sow_date") %>% dplyr::select(-gs_day)  %>%
...
df_growth_stages_lengths1_i1 <- t(dplyr::select(df_growth_stages_lengths_i1, name, length)) %>% data.frame()
...
onset_max_i1 <- max(dplyr::select(df_onset_i1, -cell, -x, -y)) # gets the maximum value of the julian day numbers
...
Soil drainage has different impacts on the suitability for an innovation that changes according to the crop that is being cultivated as well as the texture of the soil.

Sys _et al._ (1993), give suitability values for different textual drainage classes (e.g. imperfect or poor drainage). The suitability values are given for: (i) medium and fine textured soils; and (ii) coarse textured soils (sandy families). 

We record the suitability values for both soil texture types and apply these only for the relevant soil texture.

We reclassify the soil texture into the 2 classes relevant for drainage:

USDA Texture Class| New Texture Class for Drainage
------------------|---------------
clay              |	medium and fine             
silty clay        |	medium and fine           
sandy  clay       |	medium and fine       
clay loam         |	medium and fine 
silty clay loam   |	medium and fine 
sandy clay loam   |	medium and fine 
loam              |	medium and fine 
silty loam        |	medium and fine 
sandy loam        |	coarse          
silt              |	medium and fine 
loamy sand        |	coarse     
sand              |	coarse

This produces a raster called soil_TEX_4_drainage_eth.tif

We use this dataset to add more classes to the soil drainage dataset, these classes are then linked to the soil drainage requirements table. Only drainage classes in coarse soil areas are reclassified.
...
soil_drainage_fine_coarse_eth.tif

Example of drainage text file:

AfSIS_Code | IRM_Value
-----------|------------
1	         | 0.125
2	         | 0.4
3	         | 0.725
...
```{r i1_reclass_soil_drainage, results='asis'}

soil_drainage_filename_i1 <-
  as.character(paste(
    "E:/repos/raise_fs/shiny/data/",
    params$INN1,
    "_drainage_s10.csv",
...
cat("soil_drainage_filename_i1")
print(soil_drainage_filename_i1)
cat("print(file.exists(soil_drainage_filename_i1))")
print(file.exists(soil_drainage_filename_i1))



if (file.exists(soil_drainage_filename_i1)) {
  
  cat("EXISTS")
  
  
  df_soil_drainage_i1 <- read.csv(soil_drainage_filename_i1)
  

  
  df_soil_drainage_i1 %>%
    kable(digits = 3, caption = "Soil Drainage ") %>% kable_styling("striped", full_width = T) 

}

if (file.exists(soil_drainage_filename_i1)) {
  
  # cat("print(file.exists(df_raster_data_soil_i1))")
  # print(df_raster_data_soil_i1)
  
  df_requirements_drainage_i1 <-
    dplyr::filter(df_raster_data_soil_i1, drainage == 1) # filter temperature criteria
  # reclass using df_soil_texture table
  
  # cat("print(file.exists(df_requirements_texture_i1))")
  # print(df_requirements_texture_i1)
  
  if (nrow(df_requirements_drainage_i1) > 0) {
    
    cat("\n\n\n")
    cat("reclassify the drainage raster")
    cat("\n\n\n")
    
    
    #reclassify the drainage raster (ensure that it is integer)
    rast_drn_clas_rsmp_i1 <- get(paste0("rast_", df_requirements_drainage_i1[1, 12], "_rsmp_i1")) %>% as.int() %>% classify(as.matrix(
      dplyr::select(df_soil_drainage_i1, IRM_Value)
    ))
    
    plot(rast_drn_clas_rsmp_i1)
    
    #replace the texture raster in df_raster_data_soil
    text_rownum_i1 <- which(df_raster_data_soil_i1$drainage == 1)
    
    cat("\n\n\n")
    cat("text_rownum_i1")
    cat("\n\n\n")
    
    df_raster_data_soil_i1[text_rownum_i1, 12] <- "drn_clas"
...
if (file.exists(soil_texture_filename_i1)) {
  ...
  if (nrow(df_requirements_texture_i1) > 0) {
    ...
    cat("\n\n\n")
    cat("reclassify the texture raster")
    cat("\n\n\n")
    ...
    df_raster_data_soil_i1[text_rownum_i1, 12] <- "txt_clas"
...
df_requirements_single_i1 <- inner_join(df_requirements_i1, dplyr::select(df_raster_data_single_i1, data_file_prefix), by = "data_file_prefix")
...
if (params$INT == 1)
  shiny::setProgress(0.20, message = "Evaluating lower level rule bases")  #
...
```{r i1_evaluate_lower_level_rules_prop_conc, time_it = TRUE, out.width="100%", results = "asis"}
...
```{r i1_evaluate_lower_level_rules_main function, time_it = TRUE, out.width="100%", results = "asis"}
cat("\n\n\n")
...
apply(df_leaves_data_i1, 1, function(df_leaves_row_i1) {
  ...
  ## ADDED 28-07-2025
       ...
       tryCatch({
    # Debug print
    cat("\n\n\n ### Processing df_leaves_row_i1:", df_leaves_row_i1, "\n")
  ...
  #                  as.character(df_leaves_row_i1["crit_code"])), envir = results_env)))
    ...
    print(paste0("rb_", as.character(df_leaves_row_i1["crit_code"]))))
  ...
  assign(paste0("fpm_", as.character(df_leaves_row_i1["crit_code"])), eval(parse(text = x)))
   assign(paste0("fpm_", as.character(df_leaves_row_i1["crit_code"])), eval(parse(text = x)), .GlobalEnv)
   
  fpm_loop <- eval(parse(text = x))
  
  
  cat("\n\n\n")
  cat("\n",paste0("fpm_", as.character(df_leaves_row_i1["crit_code"])),"\n")
  cat("\n\n\n")
  
  # print(get(paste0("fpm_", as.character(df_leaves_row_i1["crit_code"])), envir = results_env))
  print(get(paste0("fpm_", as.character(df_leaves_row_i1["crit_code"]))))
 
  cat("\n\n\n")
  cat("\n",paste0("str(fpm_", as.character(df_leaves_row_i1["crit_code"])),")\n")
  cat("\n\n\n")
  
  # str((get(paste0("fpm_", as.character(df_leaves_row_i1["crit_code"])), envir = results_env)))
  str(get(paste0("fpm_", as.character(df_leaves_row_i1["crit_code"]))))
...
fpm_values_i1 <- function(conc_value_i1) {
  ...
  # )
    ...
    # assign(paste("result_list_i1"), c(
    #   get(paste("result_list_i1"), envir = results_env),
    #   setNames(list(eval(parse(
    #     text = x
    #   ))), conc_col_name_i1)
    # ))
    ...
    result_list_i1 <<-  c(get(paste("result_list_i1")), setNames(list(eval(parse(
      text = x
    ))), conc_col_name_i1))
    
    cat("\n\n\n")
    cat("\nprint(ls(result_list_i1))\n")
    cat("\n\n\n")
    print(ls(result_list_i1))
  ...
  str(result_list_i1)
    ...
    cat("Error in df_leaves_row_i1:", df_leaves_row_i1, "\n")
...
```{r i1_evaluate_lower_level_rules_write_data, time_it = TRUE, out.width="100%", results = "asis"}
...
if (params$INT == 1)
  shiny::setProgress(0.5, message = "Evaluating higher level rule bases")  #

```

# Objects available?

```{r objects_available_1, out.width="100%", results = 'asis'}
...
```{r objects_available_2, out.width="100%", results = 'asis'}

cat("\n\n\n")
cat("\nRule base example\n\n")
cat("\n\n\n")
print(get(paste("rb_AEK"), .GlobalEnv))
...
for (i in hierarchy_level_max_i1:1) {
  ...
  assign(paste0("df_hierarchy_i1_", i),
         inner_join(
           dplyr::select(df_rulebases_i1, crit_code, conc_level_1, conc_level_2, conc_level_3),
  ...
  if (nrow(get(paste0("df_hierarchy_i1_", i)) != 0)) {
    ...
    assign(paste0("df_one_many_i1_", i),
           apply(get(paste0("df_hierarchy_i1_", i)), 1, eval_rule_base_i1), .GlobalEnv)
...
#knitr::knit_exit()
if (params$INT == 1)
  shiny::setProgress(0.65, message = "Producing classified maps")
...
if (params$INT == 1)
  shiny::setProgress(0.75, message = "Calculating sub-division statistics") 
```
# Objects available?

```{r objects_available_3, out.width="100%", results = 'asis'}

# str(get(paste("rast_clas_fao_catFarm Production Feasibility")))
# plot(get(paste("rast_clas_fao_catFarm Production Feasibility")))
# str(get(paste("rast_clas_fao_catMarket Access")))
# plot(get(paste("rast_clas_fao_catMarket Access")))
# str(get(paste("rast_clas_fao_catClimatic Aptitude")))
# plot(get(paste("rast_clas_fao_catClimatic Aptitude")))
# str(get(paste("rast_clas_fao_catLandscape Aptitude")))
# plot(get(paste("rast_clas_fao_catLandscape Aptitude")))
# str(get(paste("rast_clas_fao_catSoil Fertility Aptitude")))
# plot(get(paste("rast_clas_fao_catSoil Fertility Aptitude")))
# str(get(paste("rast_clas_fao_catSoil Physical Properties Aptitude")))
# plot(get(paste("rast_clas_fao_catSoil Physical Properties Aptitude")))  
# str(get(paste("rast_clas_fao_catSocio-Economic Feasibility")))
# plot(get(paste("rast_clas_fao_catSocio-Economic Feasibility")))
# str(get(paste("rast_clas_fao_catBiophysical Aptitude")))
# plot(get(paste("rast_clas_fao_catBiophysical Aptitude")))
# str(get(paste("rast_clas_fao_catLikelihood of Adoption")))
# plot(get(paste("rast_clas_fao_catLikelihood of Adoption")))


cat("\n\n\n")
cat("\n\n\n length(list_rast_clas_FAO_cat):\n\n")
print(length(list_rast_clas_FAO_cat))
cat("\n\n\n")
cat("\n\n\n")
cat("\n\n\n str(list_rast_clas_FAO_cat):\n\n")
print(str(list_rast_clas_FAO_cat))
cat("\n\n\n")


# for (i in seq_along(list_rast_clas_FAO_cat)) {
# 
#   str(list_rast_clas_FAO_cat[[i]])
#   plot(list_rast_clas_FAO_cat[[i]], main = paste("Raster", i))
# 
# }

for (nm in names(list_rast_clas_FAO_cat)) {
  plot(list_rast_clas_FAO_cat[[nm]], main = nm)
}
...
# 
# 
# #raster_list <- mget(paste("list_rast_clas_FAO_cat"), .GlobalEnv)
# # Step 1: get the character vector of raster names
# raster_names <- list_rast_clas_FAO_cat  # already a list
# raster_names <- unlist(raster_names)    # make it a simple character vector
# 
# # Step 2: get the actual SpatRaster objects
...
#      
#      
# # Now you can plot each raster
# for (i in seq_along(raster_list)) {
#   r <- get(raster_list[i])
#   
#   print(str(r))
#   
#   if (!inherits(r, "SpatRaster") || all(!is.finite(values(r)))) {
#     message(paste("Raster", i, "is invalid or has no finite values. Skipping."))
#     next
#   }
#   
#   plot(r, main = raster_list[i])
# 
# }
# 
# # #plot(get(paste("list_rast_clas_FAO_cat"))[[1]])
# # 
# #  for (i in seq_along(list_rast_clas_FAO_cat)) {
# #    str(names(get(paste("list_rast_clas_FAO_cat"))[[i]]))
# #    #plot(get(paste("list_rast_clas_FAO_cat"))[[i]], main = paste("Raster", i))
# # 
# #  }
...
```{r i1_subdiv_area_FAO_palettes, results = 'asis', out.width="100%"}
...
# cat("\n\n\n")
# cat("\nprint(ls(list_rast_clas_FAO_cat))\n\n")
# cat("\n\n\n")
# print(ls(list_rast_clas_FAO_cat))
...
#for (i in 1:length(get(paste("list_rast_clas_FAO_cat"), .GlobalEnv)))   {

# for (i in 1:length(list_rast_clas_FAO_cat))  {
for (i in names(list_rast_clas_FAO_cat)) {  
  
   tryCatch({
    
     # cat("\n\n\n")
     # cat("\n\n\n ### str(get(paste(list_rast_clas_FAO_cat)))\n\n")
     # cat("\n\n\n")
     # str(get(paste("list_rast_clas_FAO_cat")))
     # cat("\n\n\n")
     #    
     # # Debug print
     # cat("\n\n\n ### Processing raster - global:", names(get(paste("list_rast_clas_FAO_cat"), .GlobalEnv)[i]), "\n")
     # 
     # # Debug print
     # cat("\n\n\n ### Processing raster:", names(get(paste("list_rast_clas_FAO_cat"))[i]), "\n\n")
     # cat("\n\n\n")
     # 
     # cat("\n--\n")
     # 
     # cat("\n\n\n")
     # cat("\n\n\n ### Plot raster - global:", names(get(paste(
     #   "list_rast_clas_FAO_cat"
     # ))[i]), "\n\n")
     # cat("\n\n\n")
     # 
     # #plot(names(get(paste("list_rast_clas_FAO_cat"), .GlobalEnv ))[i])
     # plot(get(paste("list_rast_clas_FAO_cat"), .GlobalEnv ))[i]
     # 
     # 
     # cat("\n\n\n")
     # cat("\n\n\n ### Plot raster:", names(get(paste(
     #   "list_rast_clas_FAO_cat"
     # )))[i], "\n\n")
     # cat("\n\n\n")
     # 
     # plot(names(get(paste("list_rast_clas_FAO_cat")))[i])    
  
  plot(list_rast_clas_FAO_cat[[i]], main = i)
     
  # get the name of the criteria from the raster name - this is used in the plotting
  subdiv_criteria_name_i1 <- i
  ...
  rast_to_plot_i1 <- list_rast_clas_FAO_cat[[i]]
  ...
  # get the names of the FAO classes
  FAO <- as.vector(unique(df_extr_summary_i1$FAO))
  FAO <- FAO[!is.na(FAO)]
  
  df_extr_summary_i1_pivot <- df_extr_summary_i1 %>% pivot_wider(names_from = FAO, values_from = n)
  
  # join the summary data frame to the original subdivision data spatvector
  vect_subdiv_extr_i1 <<-
    left_join(vect_subdiv_i1, df_extr_summary_i1_pivot, by = 'ID')
    
      # save as geojson and csv files
  output_vect(vect_subdiv_extr_i1, paste0(subdiv_criteria_name_i1, "_FAO_area_", params$INN1)) # save as a geojson file
  output_csv(as_tibble(vect_subdiv_extr_i1), paste0(subdiv_criteria_name_i1, "_FAO_area_", params$INN1)) # save as a csv file 
  

  # loop through each existing class type 
  for (class_type in FAO) {
    
    # Get the current iteration number
    j_i1 <- match(class_type, FAO)
    ...
    # Filter the joined data for the current limitation
    filtered_data_i1 <-
      vect_subdiv_extr_i1 %>% tidyterra::select(any_of(class_type))
    #print(filtered_data)
    ...
    subdiv_plot_i1 <- ggplot() +
      geom_spatvector(data = filtered_data_i1, aes(group = params$SUBDIVNAMEVAR, fill = eval(parse(text = FAO[j_i1])))) +
      labs(
    ...
    cat("Error in rast:", names(list_rast_clas_FAO_cat)[i], "\n")
    cat("Message:", conditionMessage(e), "\n")
    return(NULL)  # or some placeholder
})  
...
```{r objects_available_4, out.width="100%", results = 'asis'}
...
cat("\n\n\n")
cat("\n\n\n length(list_rast_limits_max_FAO):\n\n")
print(length(list_rast_limits_max_FAO))
cat("\n\n\n")
cat("\n\n\n")
cat("\n\n\n str(list_rast_limits_max_FAO):\n\n")
print(str(list_rast_limits_max_FAO))
cat("\n\n\n")
...
for (nm in names(list_rast_limits_max_FAO)) {
  plot(list_rast_limits_max_FAO[[nm]], main = nm)
}
...
#for (i in 1:length(list_rast_limits_max_FAO))   {
  for (i in names(list_rast_limits_max_FAO)) {  
  ...
  rast_to_plot_i1 <- list_rast_limits_max_FAO[[i]]
  ...
  # get the names of the FAO limitations
  FAO_limits <- as.vector(unique(df_extr_summary_i1$FAO_limit))
    FAO_limits <- FAO_limits[!is.na(FAO_limits)]
    
  # pivot wider to give a column for each limit
  df_extr_summary_i1_pivot <- df_extr_summary_i1 %>% pivot_wider(names_from = FAO_limit, values_from = n)
  ...
  vect_subdiv_extr_i1 <<-
    left_join(vect_subdiv_extr_i1, df_subdiv_area_i1, by = 'ID')
      ...
      # save as geojson and csv files
  output_vect(vect_subdiv_extr_i1, paste0(subdiv_criteria_name_i1, "_FAO_limits_area_", params$INN1)) # save as a geojson file
  output_csv(as_tibble(vect_subdiv_extr_i1), paste0(subdiv_criteria_name_i1, "_FAO_limits_area_", params$INN1)) # save as a csv file 
  

  # loop through each existing limitation type 
  for (limitation_type_i1 in FAO_limits) {
    
    # Get the current iteration number
    j_i1 <- match(limitation_type_i1, FAO_limits)
    ...
    # Filter the joined data for the current limitation
    filtered_data_i1 <-
      vect_subdiv_extr_i1 %>% tidyterra::select(any_of(limitation_type_i1))
    #print(filtered_data)
    if (nrow(filtered_data_i1) == 0) {next} 
    ...
    subdiv_plot_i1 <- ggplot() +
      geom_spatvector(data = filtered_data_i1, aes(group = params$SUBDIVNAMEVAR, fill = eval(parse(text = FAO_limits[j_i1])))) +
      labs(
...
for (i in names(list_rast_clas_FAO_cat)) {  
  ...
  # vect_subdiv_extr_i1 <<- vect_subdiv_extr_i1 %>%
  #    tidyterra::select(ID, ha, any_of(FAO)) %>% mutate(across(any_of(FAO), ~ (.x / ha) * 100)) 
  
    vect_subdiv_extr_i1 <<- vect_subdiv_extr_i1 %>% mutate(across(any_of(FAO), ~ (.x / ha) * 100)) #this includes all columns from vect_subdiv_extr_i1 A Farrow 22/05/2024
      
      # save as geojson and csv files
  output_vect(vect_subdiv_extr_i1, paste0(subdiv_criteria_name_i1, "_FAO_pc_", params$INN1)) # save as a geojson file
  output_csv(as_tibble(vect_subdiv_extr_i1), paste0(subdiv_criteria_name_i1, "_FAO_pc_", params$INN1)) # save as a csv file 
...
for (i in names(list_rast_limits_max_FAO)) {
  ...
  # vect_subdiv_extr_i1 <<- vect_subdiv_extr_i1 %>%
  #    tidyterra::select(ID, ha, any_of(FAO_limits)) %>% mutate(across(any_of(FAO_limits), ~ (.x / ha) * 100))
  
      vect_subdiv_extr_i1 <<- vect_subdiv_extr_i1 %>% mutate(across(any_of(FAO_limits), ~ (.x / ha) * 100)) #this includes all columns from vect_subdiv_extr_i1 A Farrow 22/05/2024
      

  # save as geojson and csv files
  output_vect(vect_subdiv_extr_i1, paste0(subdiv_criteria_name_i1, "_FAO_limits_pc_", params$INN1)) # save as a geojson file
  output_csv(as_tibble(vect_subdiv_extr_i1), paste0(subdiv_criteria_name_i1, "_FAO_limits_pc_", params$INN1)) # save as a csv file 
...
if (params$INT == 1)
  shiny::setProgress(0.9, message = "Calculating yield and production")
```
...
<div class="fold o"> 
```{r i1_calc_yield_pc, results = 'asis', out.width="100%"}

# get the criteria that contribute to yield

df_yield_rb_1_i1 <- dplyr::select(df_requirements_i1, "crit_code","criterion","yield") %>% dplyr::filter(!is.na(yield))
...
cat("\n\n\n")
cat("\nsummarise(df_irm_i1, Average = mean(yieldpc, na.rm = T))\n")
cat("\n\n\n")
print(summarise(df_irm_i1, Average = mean(yieldpc, na.rm = T)))
...
if (summarise(df_irm_i1, Average = mean(yieldpc, na.rm = T)) > 0) {
  ...
  vect_yieldpc_i1 <- dplyr::select(df_irm_i1, x, y, yieldpc) %>% na.omit %>% vect(geom = c("x", "y"))
  
  rast_yieldpc_i1 <- rasterize(vect_yieldpc_i1, rast_mask_proj_i1, field =
                                 "yieldpc")
  names(rast_yieldpc_i1) <- c('yieldpc')
  
  # get the cellsize for each raster
  rast_yield_pc_cellsize_i1 <- cellSize(rast_yieldpc_i1, unit = "ha")
  
  # export
  output_geotiff(rast_yieldpc_i1, paste0("yieldpc_", params$INN1))
  
  # extract the yield raster to a data frame using the subdivisions
  df_extr_i1 <-
    terra::extract(
      rast_yieldpc_i1,
      vect_subdiv_i1,
  ...
  # summarise the yield for each subdivision raster ID
  df_extr_summary_i1 <- df_extr_i1 %>%
    group_by(ID) %>%
    summarise(yieldpc = weighted.mean(yieldpc, (area * weight), na.rm = TRUE))
  ...
  # save as geojson and csv files
  output_vect(vect_subdiv_extr_i1, paste0("yieldpc_", params$INN1)) # save as a geojson file
  output_csv(as_tibble(vect_subdiv_extr_i1),
             paste0("yieldpc_", params$INN1)) # save as a csv file
...
<div class="fold o"> 
```{r i1_calc_yield, results = 'asis', out.width="100%"}

# get the criteria that contribute to yield

filename_yield_i1 <-
  as.character(paste("E:/repos/raise_fs/shiny/data/", params$INN1, "_yield_s11.csv", sep = ""))
df_yield_values_i1 <- read.csv(filename_yield_i1, na.strings = c("NA"))
...
if (summarise(df_irm_i1, Average = mean(yield, na.rm = T)) > 0) {
  ...
  vect_yield_i1 <-
    dplyr::select(df_irm_i1, x, y, yield) %>% na.omit %>% vect(geom = c("x", "y"))
  ...
  # get the cellsize for each raster
  rast_yield_cellsize_i1 <- cellSize(rast_yield_i1, unit = "ha")
  ...
  df_extr_summary_i1 <- df_extr_i1 %>%
    group_by(ID) %>%
    summarise(yield = weighted.mean(yield, (area * weight), na.rm = TRUE))
  ...
  # save as geojson and csv files
  output_vect(vect_subdiv_extr_i1, paste0("yield_", params$INN1)) # save as a geojson file
  output_csv(as_tibble(vect_subdiv_extr_i1),
             paste0("yield_", params$INN1)) # save as a csv file 
...
<div class="fold o">
```{r i1_calc_production, results = 'asis', out.width="100%"}
...
df_irm_i1$production <- df_irm_i1$yield *  Stat_factor_ha_i1

if (summarise(df_irm_i1, Average = mean(production, na.rm = T)) > 0) {
  vect_production_i1 <-
    dplyr::select(df_irm_i1, x, y, production) %>% na.omit %>% vect(geom = c("x", "y"))
  ...
  # get the cellsize for each raster
  rast_production_cellsize_i1 <- cellSize(rast_production_i1, unit = "ha")
  
  # extract the production raster to a data frame using the subdivisions
  df_extr_i1 <-
    terra::extract(
      rast_production_i1,
      vect_subdiv_i1,
  ...
  # summarise the production for each subdivision raster ID
  df_extr_summary_i1 <- df_extr_i1 %>%
    group_by(ID) %>%
    summarise(production = sum(production * weight, na.rm = TRUE))
  ...
  # save as geojson and csv files
  output_vect(vect_subdiv_extr_i1, paste0("production_", params$INN1)) # save as a geojson file
  output_csv(as_tibble(vect_subdiv_extr_i1),
             paste0("production_", params$INN1)) # save as a csv file  
  #  dp <- dBp %>%
  #    as.data.frame(xy = TRUE) %>%
  #    na.omit
...
if (params$INT == 1)
  shiny::setProgress(0.95, message = "Triangulation")

```
</div>


```{r i1_export_df_irm, results = 'asis', out.width="100%"}

output_csv(df_irm_i1, paste0("df_irm_i1_", params$INN1)) # save as a csv file

# project to geo for export as geojson
vect_irm_i1 <-  df_irm_i1 %>% na.omit %>% vect(geom = c("x", "y"), crs = wkt_lam)

vect_irm_i1_geo <- project(vect_irm_i1, "epsg:4326")

output_vect(vect_irm_i1_geo, paste0("vect_irm_i1", params$INN1)) # save as a geojson file  
...
The objective of triangulation is to evaluate the recommendation domain maps. 

Direct validation of the maps is not possible because we refer to the potential conditions that are not directly observable. Instead, the focus is on gathering perceptions among local experts - farmers and extension agents - regarding the suitability of the innovation and the limits to adoption. We can then compare the local assessment of suitability with the results of the model.

Three perceptions are captured: 

1) the general suitability of each sub-division 
2) the limits to production and therefore adoption
3) who makes the decison to adopt the innovation

## Suitability Perceptions

Local experts are asked to provide both a qualitative and quantitative measure of their perception of the suitability of a specific sub-division for the innovation that is being modelled.

### Qualitative Perception

There are three levels of suitability that the local expert can assign:

(i) not suitable
(ii) partially suitable
(iii) suitable

<div class="fold o">
```{r i1_triangulation_suit_qual, results = 'asis', out.width="100%"}
#  TRISUIT1: "SUITABILITY"

if (params$TRIANGULATION ==1) {
  expr <- paste0("vect_triangulation_farmers_i1$", params$TRISUIT1)
  # vect_triangulation_farmers_i1$tri_label_suit <-
  #   paste(vect_triangulation_farmers_i1$id, "=", eval(parse(text = (expr))))
  # vect_triangulation_farmers_i1$tri_label_suit <-
  #   factor(vect_triangulation_farmers_i1$tri_label_suit)
  
  vect_triangulation_farmers_i1$tri_suit <-   eval(parse(text = (expr)))
  vect_triangulation_farmers_i1$tri_suit <-   factor(vect_triangulation_farmers_i1$tri_suit)
  
  g_suit_farmers_i1 <- ggplot() +
    labs(title = paste0("Suitability\nFarmers Perception\n", params$INN1))
  #gtriangulation_suit_i1 <- add_triangulation_plot_no_labels_suit(g_suit_i1, vect_triangulation_farmers_i1)
  gtriangulation_suit_farmers_i1 <- add_triangulation_plot_suit(
    g_suit_farmers_i1,
    vect_triangulation_farmers_i1,
    nudge_xvaltri_farmers_i1,
    nudge_yvaltri_farmers_i1
  )
  gtriangulation_suit_farmers_i1 <- add_subdiv_simple_plot(gtriangulation_suit_farmers_i1, vect_subdiv_i1)
  gtriangulation_suit_farmers_i1
}

if (params$TRIANGULATION ==1) {
  expr <- paste0("vect_triangulation_agents_i1$", params$TRISUIT1)
  # vect_triangulation_agents_i1$tri_label_suit <-
  #   paste(vect_triangulation_agents_i1$id, "=", eval(parse(text = (expr))))
  # vect_triangulation_agents_i1$tri_label_suit <-
  #   factor(vect_triangulation_agents_i1$tri_label_suit)
  
  vect_triangulation_agents_i1$tri_suit <-   eval(parse(text = (expr)))
  vect_triangulation_agents_i1$tri_suit <-   factor(vect_triangulation_agents_i1$tri_suit)
  
  g_suit_agents_i1 <- ggplot() +
    labs(title = paste0("Suitability\nAgents Perception\n", params$INN1))
  #gtriangulation_suit_i1 <- add_triangulation_plot_no_labels_suit(g_suit_i1, vect_triangulation_agents_i1)
  gtriangulation_suit_agents_i1 <- add_triangulation_plot_suit(
    g_suit_agents_i1,
    vect_triangulation_agents_i1,
    nudge_xvaltri_agents_i1,
    nudge_yvaltri_agents_i1
  )
  gtriangulation_suit_agents_i1 <- add_subdiv_simple_plot(gtriangulation_suit_agents_i1, vect_subdiv_i1)
  gtriangulation_suit_agents_i1
}
...
The local expert is also asked to assign a percentage to the area within the sub-division which is suitable.

<div class="fold o">
```{r i1_triangulation_suit_quan, results = 'asis', out.width="100%"}
#  TRIPCS1: "PC_SUIT"

if (params$TRIANGULATION ==1) {
  expr <- paste0("vect_triangulation_farmers_i1$", params$TRIPCS1)
  # vect_triangulation_farmers_i1$tri_label_pcs <-
  #   paste(vect_triangulation_farmers_i1$id, "=", eval(parse(text = (expr))))
  # vect_triangulation_farmers_i1$tri_label_pcs <-
  #   factor(vect_triangulation_farmers_i1$tri_label_pcs)
  
  vect_triangulation_farmers_i1$tri_pcs <-   eval(parse(text = (expr)))
  
  g_pcs_farmers_i1 <- ggplot() +
      labs(title = paste0("% Area Suitable\nFarmers Perception\n", params$INN1))
  #gtriangulation_pcs_i1 <- add_triangulation_plot_no_labels_pcs(g_pcs_i1, vect_triangulation_farmers_i1)
  gtriangulation_pcs_farmers_i1 <- add_triangulation_plot_pcs(g_pcs_farmers_i1, vect_triangulation_farmers_i1,
                                                    nudge_xvaltri_farmers_i1,
                                                    nudge_yvaltri_farmers_i1)
  gtriangulation_pcs_farmers_i1 <- add_subdiv_simple_plot(gtriangulation_pcs_farmers_i1, vect_subdiv_i1)
  gtriangulation_pcs_farmers_i1
}

if (params$TRIANGULATION ==1) {
  expr <- paste0("vect_triangulation_agents_i1$", params$TRIPCS1)
  # vect_triangulation_agents_i1$tri_label_pcs <-
  #   paste(vect_triangulation_agents_i1$id, "=", eval(parse(text = (expr))))
  # vect_triangulation_agents_i1$tri_label_pcs <-
  #   factor(vect_triangulation_agents_i1$tri_label_pcs)
  
  vect_triangulation_agents_i1$tri_pcs <-   eval(parse(text = (expr)))
  
  g_pcs_agents_i1 <- ggplot() +
      labs(title = paste0("% Area Suitable\nAgents Perception\n", params$INN1))
  #gtriangulation_pcs_i1 <- add_triangulation_plot_no_labels_pcs(g_pcs_i1, vect_triangulation_agents_i1)
  gtriangulation_pcs_agents_i1 <- add_triangulation_plot_pcs(g_pcs_agents_i1, vect_triangulation_agents_i1,
                                                    nudge_xvaltri_agents_i1,
                                                    nudge_yvaltri_agents_i1)
  gtriangulation_pcs_agents_i1 <- add_subdiv_simple_plot(gtriangulation_pcs_agents_i1, vect_subdiv_i1)
  gtriangulation_pcs_agents_i1
}

```
</div>

## Limits to production and adoption

The expert is asked to rank key factors that limit adoption. These factors are split into factors that affect the biophysical aptitude of the kebele, and those factors that affect the socio-economic feasibility of the kebele.

The user chooses the most important factor, then the second-most important factor etc. until all factors have been assigned. If the expert doesn't know then they can leave the rankings blank and choose a checkbox input. Likewise they can also choose a different type of factor that is not in the list.

The factors are the following (the codes are used for mapping and match as closely as possible the codes used in the IRM model):

Bio-physical aptitude

* Pests (Ps)
* Diseases (Ds)
* Low soil fertility (SF)
* Rainfall (P)
* Temperature (T)
* Drainage and Flooding (Dr)
* Slope and Terrain (Ls)
* I don't know (true or false)
* Other (free text)

Socio-economic feasibility

* Lack of quality seeds (Sd)
* Lack of fertilizers/pesticides (In)
* Limited market access (MA)
* No demand from the market (MD)
* Financial constraints (F)
* Labour shortage (Lb)
* Lack of knowledge of the innovation (Ex)
* I don't know (true or false)
* Other (free text)
...
<div class="fold o">
```{r i1_triangulation_limits_farmers_dominant_ba, results = 'asis', out.width="100%"}

#  TRILIMITS1: "LIMITS"

if (params$TRIANGULATION ==1) {
  vect_triangulation_farmers_i1$LIM_BA_1 <-   factor(vect_triangulation_farmers_i1$LIM_BA_1)
  
  g_lim_farmers_i1 <- ggplot() +
    labs(title = paste0("Most Important Biophysical Limitation\nFarmers Perception\n", params$INN1))
  
  gtriangulation_lim_1_farmers_i1 <- add_triangulation_plot_lim_1_ba(
    g_lim_farmers_i1,
    vect_triangulation_farmers_i1,
    nudge_xvaltri_farmers_i1,
    nudge_yvaltri_farmers_i1
  )
  gtriangulation_lim_1_farmers_i1 <- add_subdiv_simple_plot(gtriangulation_lim_1_farmers_i1, vect_subdiv_i1)
  gtriangulation_lim_1_farmers_i1
}
...
<div class="fold o">
```{r i1_triangulation_limits_farmers_other_ba, results = 'asis', out.width="100%"}

#  TRILIMITS1: "LIMITS"

if (params$TRIANGULATION ==1) {

  vect_triangulation_farmers_i1$LIM_BA_OTH <-   factor(vect_triangulation_farmers_i1$LIM_BA_OTH) 
  
  g_lim_farmers_i1_ba_oth <- ggplot(vect_triangulation_farmers_i1) +
    geom_spatvector(
      aes(fill = LIM_BA_OTH),
      colour = "black",
      shape = 21,
    ...
    geom_spatvector_label(
      #aes(label = params$SUBDIVNAMEVAR),
      aes(label =.data[[ params$SUBDIVNAMEVAR]]),
      colour = "black",
      size = 2,
      alpha = 0.5,
      nudge_x = nudge_xvaltri_farmers_i1,
      nudge_y = nudge_yvaltri_farmers_i1
    ) +
    scale_fill_discrete(na.value = "white") +
    #theme_minimal() +
    labs(title = paste0("Other Biophysical Limitation\nFarmers Perception\n", params$INN1),
         fill = "Limitation")
   
  

  g_lim_farmers_i1_ba_oth <- add_subdiv_simple_plot(g_lim_farmers_i1_ba_oth, vect_subdiv_i1)
  g_lim_farmers_i1_ba_oth
}

```
</div>

##### Importance of individual limitation

<div class="fold o">
```{r i1_triangulation_limits_farmers_byfactor_ba, results = 'asis', out.width="100%"}
if (params$TRIANGULATION ==1) {
# List of rank variables
limitation_ba_vars <- paste0("LIM_BA_", 1:7)

# Extract attributes
df_triangulation_farmers_i1 <- as.data.frame(vect_triangulation_farmers_i1)

# Map ranks to scores (7 → 1)
rank_ba_scores <- setNames(7:1, limitation_ba_vars)

# Long format with scores
df_triangulation_farmers_i1_long_ba <- df_triangulation_farmers_i1 %>%
  pivot_longer(cols = all_of(limitation_ba_vars),
               names_to = "rank_ba",
               values_to = "limitation_ba") %>%
  mutate(ba_score = rank_ba_scores[rank_ba])

# Wide format: one column per limitation code, values = score
df_triangulation_farmers_i1_wide_ba <- df_triangulation_farmers_i1_long_ba %>%
  select(params$SUBDIVNAMEVAR, limitation_ba, ba_score) %>%
  tidyterra::filter(!is.na(limitation_ba)) %>%   # drop empty entries
  pivot_wider(names_from = limitation_ba,
              values_from = ba_score,
              values_fn = max,     # keep highest if duplicate
              values_fill = NA)

# Convert all score columns to factors with descriptive labels
importance_labels <- c("Most important",
                       "Very high",
                       "High",
                       "Medium",
                       "Low",
...
limitation_codes <- setdiff(names(df_triangulation_farmers_i1_wide_ba), params$SUBDIVNAMEVAR)

for (code in limitation_codes) {
  df_triangulation_farmers_i1_wide_ba[[paste0(code, "_fac")]] <- factor(df_triangulation_farmers_i1_wide_ba[[code]],
                                               levels = 7:1,
                                               labels = importance_labels)
}

geo_scores <- merge(vect_triangulation_farmers_i1, df_triangulation_farmers_i1_wide_ba, by = params$SUBDIVNAMEVAR, all.x = TRUE)

for (code in limitation_codes) {
  
  if (code == "Dr") {plot_label = "Drainage"}
  if (code == "Ds") {plot_label = "Diseases"}
  if (code == "P") {plot_label = "Rainfall"}
  if (code == "Ps") {plot_label = "Pests"}
  if (code == "SF") {plot_label = "Soil Fertility"}
  if (code == "Ls") {plot_label = "Slope/Terrain"}
  if (code == "T") {plot_label = "Temperature"}
  
  fac_col <- paste0(code, "_fac")
  
  gtriangulation_lim_ba_farmers_i1 <- ggplot(geo_scores) +
    geom_spatvector(aes_string(fill = fac_col),
                    colour = "black", shape = 21, size = 2, show.legend=TRUE) +
    geom_spatvector_label(
      aes(label =.data[[ params$SUBDIVNAMEVAR]]),
      colour = "black",
      size = 2,
    ...
    scale_fill_viridis_d(na.value = "white",
                         option = "E",
                         drop=FALSE) +  # discrete scale for factors
    #theme_minimal() +
    labs(title = paste0("Importance of Limitation: ", plot_label, "\nFarmers Perception\n", params$INN1),
         fill = "Importance")
  
  gtriangulation_lim_ba_farmers_i1 <- add_subdiv_simple_plot(gtriangulation_lim_ba_farmers_i1, vect_subdiv_i1)
  
  print(gtriangulation_lim_ba_farmers_i1)
...
<div class="fold o">
```{r i1_triangulation_limits_farmers_dominant_se, results = 'asis', out.width="100%"}

#  TRILIMITS1: "LIMITS"

if (params$TRIANGULATION ==1) {
  vect_triangulation_farmers_i1$LIM_SE_1 <-   factor(vect_triangulation_farmers_i1$LIM_SE_1)
  
  g_lim_farmers_i1 <- ggplot() +
    labs(title = paste0("Most Important Socio-Economic Limitation\nFarmers Perception\n", params$INN1))
  
  gtriangulation_lim_1_farmers_i1 <- add_triangulation_plot_lim_1_se(
    g_lim_farmers_i1,
    vect_triangulation_farmers_i1,
    nudge_xvaltri_farmers_i1,
    nudge_yvaltri_farmers_i1
...
<div class="fold o">
```{r i1_triangulation_limits_farmers_other_se, results = 'asis', out.width="100%"}
...
vect_triangulation_farmers_i1$LIM_SE_OTH <-   factor(vect_triangulation_farmers_i1$LIM_SE_OTH)  
  
  g_lim_farmers_i1_se_oth <- ggplot(vect_triangulation_farmers_i1) +
    geom_spatvector(
      aes(fill = LIM_SE_OTH),
      colour = "black",
      shape = 21,
    ...
    scale_fill_discrete(na.value = "white")+
    #theme_minimal() +
    labs(title = paste0("Other Socio-Economic Limitation\nFarmers Perception\n", params$INN1),
         fill = "Limitation")
   
  

  g_lim_farmers_i1_se_oth <- add_subdiv_simple_plot(g_lim_farmers_i1_se_oth, vect_subdiv_i1)
  g_lim_farmers_i1_se_oth
}
...
<div class="fold o">
```{r i1_triangulation_limits_farmers_byfactor_se, results = 'asis', out.width="100%"}

if (params$TRIANGULATION ==1) {
  
# List of rank variables
limitation_se_vars <- paste0("LIM_SE_", 1:7)

# Extract attributes
df_triangulation_farmers_i1 <- as.data.frame(vect_triangulation_farmers_i1)

# Map ranks to scores (7 → 1)
rank_se_scores <- setNames(7:1, limitation_se_vars)

# Long format with scores
df_triangulation_farmers_i1_long_se <- df_triangulation_farmers_i1 %>%
  pivot_longer(cols = all_of(limitation_se_vars),
               names_to = "rank_se",
               values_to = "limitation_se") %>%
  mutate(se_score = rank_se_scores[rank_se])

# Wide format: one column per limitation code, values = score
df_triangulation_farmers_i1_wide_se <- df_triangulation_farmers_i1_long_se %>%
  select(params$SUBDIVNAMEVAR, limitation_se, se_score) %>%
  tidyterra::filter(!is.na(limitation_se)) %>%   # drop empty entries
  pivot_wider(names_from = limitation_se,
              values_from = se_score,
              values_fn = max,     # keep highest if duplicate
              values_fill = NA)
...
limitation_codes <- setdiff(names(df_triangulation_farmers_i1_wide_se), params$SUBDIVNAMEVAR)

for (code in limitation_codes) {
  df_triangulation_farmers_i1_wide_se[[paste0(code, "_fac")]] <- factor(df_triangulation_farmers_i1_wide_se[[code]],
                                               levels = 7:1,
                                               labels = importance_labels)
}

geo_scores <- merge(vect_triangulation_farmers_i1, df_triangulation_farmers_i1_wide_se, by = params$SUBDIVNAMEVAR, all.x = TRUE)

for (code in limitation_codes) {
  
  if (code == "Ex") {plot_label = "Extension/Knowledge"}
  if (code == "F") {plot_label = "Financial Constraints"}
  if (code == "In") {plot_label = "Fertilisers/Pesticides"}
  if (code == "Lb") {plot_label = "Labour"}
  if (code == "MA") {plot_label = "Market Access"}
  if (code == "MD") {plot_label = "Market Demand"}
  if (code == "Sd") {plot_label = "Access to Seeds"}
  
  fac_col <- paste0(code, "_fac")
  
  gtriangulation_lim_se_farmers_i1 <- ggplot(geo_scores) +
    geom_spatvector(aes_string(fill = fac_col),
                    colour = "black", shape = 21, size = 2, show.legend=TRUE) +
    geom_spatvector_label(
      aes(label =.data[[ params$SUBDIVNAMEVAR]]),
  ...
  gtriangulation_lim_se_farmers_i1 <- add_subdiv_simple_plot(gtriangulation_lim_se_farmers_i1, vect_subdiv_i1)
  
  print(gtriangulation_lim_se_farmers_i1)
...
<div class="fold o">
```{r i1_triangulation_limits_agents_dominant_ba, results = 'asis', out.width="100%"}

#  TRILIMITS1: "LIMITS"

if (params$TRIANGULATION ==1) {
  vect_triangulation_agents_i1$LIM_BA_1 <-   factor(vect_triangulation_agents_i1$LIM_BA_1)
  
  g_lim_agents_i1 <- ggplot() +
    labs(title = paste0("Most Important Biophysical Limitation\nAgents Perception\n", params$INN1))
  
  gtriangulation_lim_1_agents_i1 <- add_triangulation_plot_lim_1_ba(
    g_lim_agents_i1,
    vect_triangulation_agents_i1,
    nudge_xvaltri_agents_i1,
    nudge_yvaltri_agents_i1
  )
  gtriangulation_lim_1_agents_i1 <- add_subdiv_simple_plot(gtriangulation_lim_1_agents_i1, vect_subdiv_i1)
  gtriangulation_lim_1_agents_i1
}
...
<div class="fold o">
```{r i1_triangulation_limits_agents_other_ba, results = 'asis', out.width="100%"}

#  TRILIMITS1: "LIMITS"

if (params$TRIANGULATION ==1) {

  vect_triangulation_agents_i1$LIM_BA_OTH <-   factor(vect_triangulation_agents_i1$LIM_BA_OTH) 
  
  g_lim_agents_i1_ba_oth <- ggplot(vect_triangulation_agents_i1) +
    geom_spatvector(
      aes(fill = LIM_BA_OTH),
      colour = "black",
      shape = 21,
    ...
    geom_spatvector_label(
      ...
      size = 2,
      alpha = 0.5,
      nudge_x = nudge_xvaltri_agents_i1,
      nudge_y = nudge_yvaltri_agents_i1
    ) +
    scale_fill_discrete(na.value = "white") +
    #theme_minimal() +
    labs(title = paste0("Other Biophysical Limitation\nAgents Perception\n", params$INN1),
  ...
  g_lim_agents_i1_ba_oth <- add_subdiv_simple_plot(g_lim_agents_i1_ba_oth, vect_subdiv_i1)
  g_lim_agents_i1_ba_oth
}
...
<div class="fold o">
```{r i1_triangulation_limits_agents_byfactor_ba, results = 'asis', out.width="100%"}
...
# Extract attributes
df_triangulation_agents_i1 <- as.data.frame(vect_triangulation_agents_i1)

# Map ranks to scores (7 → 1)
rank_ba_scores <- setNames(7:1, limitation_ba_vars)

# Long format with scores
df_triangulation_agents_i1_long_ba <- df_triangulation_agents_i1 %>%
  pivot_longer(cols = all_of(limitation_ba_vars),
               names_to = "rank_ba",
               values_to = "limitation_ba") %>%
  mutate(ba_score = rank_ba_scores[rank_ba])
...
# Wide format: one column per limitation code, values = score
df_triangulation_agents_i1_wide_ba <- df_triangulation_agents_i1_long_ba %>%
  select(params$SUBDIVNAMEVAR, limitation_ba, ba_score) %>%
  tidyterra::filter(!is.na(limitation_ba)) %>%   # drop empty entries
  pivot_wider(names_from = limitation_ba,
              values_from = ba_score,
...
limitation_codes <- setdiff(names(df_triangulation_agents_i1_wide_ba), params$SUBDIVNAMEVAR)

for (code in limitation_codes) {
  df_triangulation_agents_i1_wide_ba[[paste0(code, "_fac")]] <- factor(df_triangulation_agents_i1_wide_ba[[code]],
                                               levels = 7:1,
                                               labels = importance_labels)
}

geo_scores <- merge(vect_triangulation_agents_i1, df_triangulation_agents_i1_wide_ba, by = params$SUBDIVNAMEVAR, all.x = TRUE)
...
for (code in limitation_codes) {
  ...
  gtriangulation_lim_ba_agents_i1 <- ggplot(geo_scores) +
    geom_spatvector(aes_string(fill = fac_col),
                    colour = "black", shape = 21, size = 2, show.legend=TRUE) +
    geom_spatvector_label(
      aes(label =.data[[ params$SUBDIVNAMEVAR]]),
  ...
  gtriangulation_lim_ba_agents_i1 <- add_subdiv_simple_plot(gtriangulation_lim_ba_agents_i1, vect_subdiv_i1)
  
  print(gtriangulation_lim_ba_agents_i1)
...
<div class="fold o">
```{r i1_triangulation_limits_agents_dominant_se, results = 'asis', out.width="100%"}

#  TRILIMITS1: "LIMITS"

if (params$TRIANGULATION ==1) {
  vect_triangulation_agents_i1$LIM_SE_1 <-   factor(vect_triangulation_agents_i1$LIM_SE_1)
  
  g_lim_agents_i1 <- ggplot() +
    labs(title = paste0("Most Important Socio-Economic Limitation\nAgents Perception\n", params$INN1))
  
  gtriangulation_lim_1_agents_i1 <- add_triangulation_plot_lim_1_se(
    g_lim_agents_i1,
    vect_triangulation_agents_i1,
    nudge_xvaltri_agents_i1,
    nudge_yvaltri_agents_i1
...
<div class="fold o">
```{r i1_triangulation_limits_agents_other_se, results = 'asis', out.width="100%"}
...
vect_triangulation_agents_i1$LIM_SE_OTH <-   factor(vect_triangulation_agents_i1$LIM_SE_OTH)  
  
  g_lim_agents_i1_se_oth <- ggplot(vect_triangulation_agents_i1) +
    geom_spatvector(
      aes(fill = LIM_SE_OTH),
      colour = "black",
      shape = 21,
  ...
  g_lim_agents_i1_se_oth <- add_subdiv_simple_plot(g_lim_agents_i1_se_oth, vect_subdiv_i1)
  g_lim_agents_i1_se_oth
}
...
<div class="fold o">
```{r i1_triangulation_limits_agents_byfactor_se, results = 'asis', out.width="100%"}
...
# Long format with scores
df_triangulation_agents_i1_long_se <- df_triangulation_agents_i1 %>%
  pivot_longer(cols = all_of(limitation_se_vars),
               names_to = "rank_se",
               values_to = "limitation_se") %>%
  mutate(se_score = rank_se_scores[rank_se])
...
# Wide format: one column per limitation code, values = score
df_triangulation_agents_i1_wide_se <- df_triangulation_agents_i1_long_se %>%
  select(params$SUBDIVNAMEVAR, limitation_se, se_score) %>%
  tidyterra::filter(!is.na(limitation_se)) %>%   # drop empty entries
  pivot_wider(names_from = limitation_se,
              values_from = se_score,
...
limitation_codes <- setdiff(names(df_triangulation_agents_i1_wide_se), params$SUBDIVNAMEVAR)

for (code in limitation_codes) {
  df_triangulation_agents_i1_wide_se[[paste0(code, "_fac")]] <- factor(df_triangulation_agents_i1_wide_se[[code]],
                                               levels = 7:1,
                                               labels = importance_labels)
}

geo_scores <- merge(vect_triangulation_agents_i1, df_triangulation_agents_i1_wide_se, by = params$SUBDIVNAMEVAR, all.x = TRUE)
...
for (code in limitation_codes) {
  ...
  gtriangulation_lim_se_agents_i1 <- ggplot(geo_scores) +
    geom_spatvector(aes_string(fill = fac_col),
                    colour = "black", shape = 21, size = 2, show.legend=TRUE) +
    geom_spatvector_label(
      aes(label =.data[[ params$SUBDIVNAMEVAR]]),
  ...
  gtriangulation_lim_se_agents_i1 <- add_subdiv_simple_plot(gtriangulation_lim_se_agents_i1, vect_subdiv_i1)
  
  print(gtriangulation_lim_se_agents_i1)
...
<div class="fold o">
```{r i1_triangulation_limits, results = 'asis', out.width="100%"}

# #  TRILIMITS1: "LIMITS"
# 
# if (!is.na(params$TRILIMITS1)) {
#   expr <- paste0("vect_triangulation_farmers_i1$", params$TRILIMITS1)
#   #print(expr)
#   #eval(parse(text = (expr)))
#   # vect_triangulation_farmers_i1$tri_label_lim <-
#   #   paste(vect_triangulation_farmers_i1$id, "=", eval(parse(text = (expr))))
#   # vect_triangulation_farmers_i1$tri_label_lim <-
#   #   factor(vect_triangulation_farmers_i1$tri_label_lim)
#   
#   vect_triangulation_farmers_i1$tri_limits <-   eval(parse(text = (expr)))
#   vect_triangulation_farmers_i1$tri_limits <-   factor(vect_triangulation_farmers_i1$tri_limits)
#   
#   g_lim_farmers_i1 <- ggplot() +
#       labs(title = paste0("\nFarmers Triangulation - Limits\n", params$INN1))
#   #gtriangulation_lim_i1 <- add_triangulation_plot_no_labels_lim(g_lim_i1, vect_triangulation_farmers_i1)
#   gtriangulation_lim_farmers_i1 <- add_triangulation_plot_lim(g_lim_farmers_i1,
#                                                     vect_triangulation_farmers_i1,
#                                                     nudge_xvaltri_farmers_i1,
#                                                     nudge_yvaltri_farmers_i1)
#   gtriangulation_lim_farmers_i1 <- add_subdiv_simple_plot(gtriangulation_lim_farmers_i1, vect_subdiv_i1)
#   gtriangulation_lim_farmers_i1
# }
# 
# 
# if (!is.na(params$TRILIMITS1)) {
#   expr <- paste0("vect_triangulation_agents_i1$", params$TRILIMITS1)
#   #print(expr)
#   #eval(parse(text = (expr)))
#   # vect_triangulation_agents_i1$tri_label_lim <-
#   #   paste(vect_triangulation_agents_i1$id, "=", eval(parse(text = (expr))))
#   # vect_triangulation_agents_i1$tri_label_lim <-
#   #   factor(vect_triangulation_agents_i1$tri_label_lim)
#   
#     
#   vect_triangulation_agents_i1$tri_limits <-   eval(parse(text = (expr)))
#   vect_triangulation_agents_i1$tri_limits <-   factor(vect_triangulation_agents_i1$tri_limits)
#   
#   g_lim_agents_i1 <- ggplot() +
#       labs(title = paste0("\nAgents Triangulation - Limits\n", params$INN1))
#   #gtriangulation_lim_i1 <- add_triangulation_plot_no_labels_lim(g_lim_i1, vect_triangulation_agents_i1)
#   gtriangulation_lim_agents_i1 <- add_triangulation_plot_lim(g_lim_agents_i1,
#                                                     vect_triangulation_agents_i1,
#                                                     nudge_xvaltri_agents_i1,
#                                                     nudge_yvaltri_agents_i1)
#   gtriangulation_lim_agents_i1 <- add_subdiv_simple_plot(gtriangulation_lim_agents_i1, vect_subdiv_i1)
#   gtriangulation_lim_agents_i1
# }
...
For each sub-division the local experts chooses who in general makes the decision to adopt the innovation. Options include:

* Farmer (self)
* Household decision
* Cooperative or group
* Advice from extension worker
* Driven by market demand
* From Agriculture office
* Other

<div class="fold o">
```{r i1_triangulation_adoptdecision, results = 'asis', out.width="100%"}

#  TRIDEC1: "DECISION"

if (params$TRIANGULATION ==1) {
  expr <- paste0("vect_triangulation_farmers_i1$", params$TRIDEC1)
  # vect_triangulation_farmers_i1$tri_label_dec <-
  #   paste(vect_triangulation_farmers_i1$id, "=", eval(parse(text = (expr))))
  # vect_triangulation_farmers_i1$tri_label_dec <-
  #   factor(vect_triangulation_farmers_i1$tri_label_dec)
    
  vect_triangulation_farmers_i1$tri_dec <-   eval(parse(text = (expr)))
  vect_triangulation_farmers_i1$tri_dec <-   factor(vect_triangulation_farmers_i1$tri_dec)
  
  
  g_dec_farmers_i1 <- ggplot() +
      labs(title = paste0("Decision to Adopt\nFarmers Perception\n", params$INN1))
  #gtriangulation_dec_i1 <- add_triangulation_plot_no_labels_dec(g_dec_i1, vect_triangulation_farmers_i1)
  gtriangulation_dec_farmers_i1 <- add_triangulation_plot_dec(g_dec_farmers_i1, vect_triangulation_farmers_i1,
                                                    nudge_xvaltri_farmers_i1,
                                                    nudge_yvaltri_farmers_i1)
  gtriangulation_dec_farmers_i1 <- add_subdiv_simple_plot(gtriangulation_dec_farmers_i1, vect_subdiv_i1)
  gtriangulation_dec_farmers_i1
}

if (!is.na(params$TRIDEC1)) {
  expr <- paste0("vect_triangulation_agents_i1$", params$TRIDEC1)
  # vect_triangulation_agents_i1$tri_label_dec <-
  #   paste(vect_triangulation_agents_i1$id, "=", eval(parse(text = (expr))))
  # vect_triangulation_agents_i1$tri_label_dec <-
  #   factor(vect_triangulation_agents_i1$tri_label_dec)
      
  vect_triangulation_agents_i1$tri_dec <-   eval(parse(text = (expr)))
  vect_triangulation_agents_i1$tri_dec <-   factor(vect_triangulation_agents_i1$tri_dec)
  
  g_dec_agents_i1 <- ggplot() +
      labs(title = paste0("Decision to Adopt\nAgents Perception\n", params$INN1))
  #gtriangulation_dec_i1 <- add_triangulation_plot_no_labels_dec(g_dec_i1, vect_triangulation_agents_i1)
  gtriangulation_dec_agents_i1 <- add_triangulation_plot_dec(g_dec_agents_i1, vect_triangulation_agents_i1,
                                                    nudge_xvaltri_agents_i1,
                                                    nudge_yvaltri_agents_i1)
  gtriangulation_dec_agents_i1 <- add_subdiv_simple_plot(gtriangulation_dec_agents_i1, vect_subdiv_i1)
  gtriangulation_dec_agents_i1
}

if (params$INT == 1)
  shiny::setProgress(1, message = "Finished!")
```
</div>
...
if (is.na(params$INN2)) {
  ...
  cat("Javascript code and css style used in this script by Martin Schmelzer
