...
for (i in 1:length(list_rast_clas_FAO_cat))   {
  ...
  for (class_type in FAO) {
    ...
    subdiv_plot_i1 <- ggplot() +
      geom_spatvector(data = filtered_data_i1, aes(group = params$SUBDIV1, fill = eval(parse(text = FAO[j_i1])))) +
      labs(
...
for (i in 1:length(list_rast_limits_max_FAO))   {
  ...
  for (limitation_type_i1 in FAO_limits) {
    ...
    subdiv_plot_i1 <- ggplot() +
      geom_spatvector(data = filtered_data_i1, aes(group = params$SUBDIV1, fill = eval(parse(text = FAO_limits[j_i1])))) +
      labs(
...
for (i in length(rb_i12):1)   {
  ...
  for (class_type in FAO) {
    ...
    subdiv_plot_i12 <- ggplot() +
      geom_spatvector(data = filtered_data_i12, aes(group = params$SUBDIV1, fill = eval(parse(text = FAO[j_i12])))) +
      labs(
...
# get the names of the FAO limitations
  ...
  for (limitation_type_i12 in FAO_limits) {
    ...
    # Create a ggplot for count value per subdivision for the current limitation type using the palette from above
    subdiv_plot_i12 <- ggplot() +
      geom_spatvector(data = filtered_data_i12, aes(group = params$SUBDIV1, fill = eval(parse(text = FAO_limits[j_i12])))) +
      labs(
