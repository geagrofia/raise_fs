...
cat("csv file = E:/repos/raise_fs/tab_data/input/transcription/T_",
    crop_name,
...
for (var in var_cols) {
  ...
  df_plot |> select(-row_id) |> t() |>
    ...
    format = "html") |> kable_styling(
      bootstrap_options = "bordered", font_size = 9,
      full_width = F,
      position = "left"
    ) |>  print()
  ...
  if (exists("cross_asc"))
  {
    if (exists("cross_desc")) { # implies two thresholds required
      ...
      if (boundary_df1 > boundary_df2) { # implies that df1 is ascending
        ...
        # add to df to later link to df_output
        df_var_output <- df_var_output |> mutate(threshold_1 = cross_desc$final_x,
...
df_output |> kable(caption = "Criteria, thresholds and boundary widths") |> kable_styling(
  bootstrap_options = "bordered",
  full_width = F,
  position = "left"
) |>  print()
...
) |> relocate(c(weight, threshold_1, threshold_2), .after = criterion)
