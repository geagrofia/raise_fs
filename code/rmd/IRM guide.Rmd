...
File             | Description
-----------------|----------------------------------
ag_land_arsi.tif | Agricultural land geotif (any CRS)
 basesat_arsi.tif  | Base saturation geotif (any CRS)
 cec_arsi.tif | CEC geotif (any CRS)
 cor_frag_arsi.tif | Coarse fragments geotif (any CRS)
 **crs_arsi_hawassa09.txt** | Text file of arsi_hawassa09 CRS in proj4 format
 depth_arsi.tif | Soil Depth geotif (any CRS)
 drain_arsi.tif | drainage (#) geotif (any CRS)
 ec_arsi.tif | electrical conductivity (dS/m) geotif (any CRS)
 ex_bases_arsi.tif | Exchangeable bases geotif (any CRS)
 **mask_arsi_hawassa09.tif** | Mask geotif (uses the arsi_hawassa09 CRS)
 pHh2o_arsi.tif | Soil pH-H2O (between 0 and 14) geotif (any CRS)
 prec_m_arsi.tif | monthly precipitation sum (mm) geotif (any CRS)
 slope_arsi.tif | slope (degrees) geotif (any CRS)
 soc_arsi.tif | soil organic carbon (g/kg) geotif (any CRS)
 srtm_arsi.tif | elevation (masl) geotif (any CRS)
 **subdiv_arsi_hawassa09.geojson** | subdivision multipart polygons of modelling area geojson (WGS84 CRS)
 tavg_m_arsi.tif | monthly mean tempreature (degC) geotif (any CRS)
 **triangulation_arsi_hawassa09.geojson** | triangulation points geojson (WGS84 CRS)
 txt_arsi.tif | soil texture (USDA #) geotif (any CRS)
*** 
...
File             | Description
-----------------|----------------------------------
apt_arsi_hawassa09.geojson | point file of all inputs and results (WGS84 CRS)
ba_nuance_subdiv_area_arsi_hawassa09.geojson |  polygon file of biophysical aptitude nuanced results in hectares per subdivision (WGS84 CRS)
ba_nuance_subdiv_pc_arsi_hawassa09.geojson |  polygon file of biophysical aptitude nuanced results in percentage of the area per subdivision (WGS84 CRS)
ba_subdiv_area_arsi_hawassa09.geojson | polygon file of biophysical aptitude results in hectares per subdivision (WGS84 CRS)
ba_subdiv_pc_arsi_hawassa09.geojson | polygon file of biophysical aptitude results in percentage of the area per subdivision (WGS84 CRS)
...
File             | Description
-----------------|----------------------------------
growth_stages_arsi_hawassa09.csv | definition of growth stage lengths, and sowing/planting date [full details...](#growth_stages-link)
**priorities_arsi_hawassa09.csv** | definition of all criteria, rule base structure, weights, thresholds, data file prefixes, rule propositions and conclusions and contribution to yield estimation [full details...](#priorities-link)
usda_texture_arsi_hawassa09.csv | table for classifying soil texture in IRM from USDA# to value [full details...](#texture-link)
*** 
Files in **bold** are obligatory
...
File             | Description
-----------------|----------------------------------
ba_nuance_subdiv_area_arsi_hawassa09.csv | table of biophysical aptitude nuanced results in hectares per subdivision (WGS84 CRS)
ba_nuance_subdiv_pc_arsi_hawassa09.csv | table of biophysical aptitude nuanced results in percentage of the area per subdivision (WGS84 CRS)
ba_subdiv_area_arsi_hawassa09.csv | table of biophysical aptitude results in hectares per subdivision (WGS84 CRS)
ba_subdiv_pc_arsi_hawassa09.csv | table of biophysical aptitude results in percentage of the area per subdivision (WGS84 CRS)


# Important files

## priorities_arsi_hawassa09.csv {#priorities-link}
...
## growth_stages_arsi_hawassa09.csv {#growth_stages-link}

Column name / Variable | Description  | No data
-----------------|----------------------------------|----------------------------------
name| short name of phenological stage consistent with 'phen_stage' in priorities csv. First value always sowing date |	Always has a value
day	| sowing date julian day number between 0 and 365 | First value always sowing date. Leave other rows blank
length	| length in days of each phenological stage. Always include total length even if not used as a criterion | First value always blank. Other rows always have a value
criteria | is this phenological stage a criterion for IRM  | First value always 0. Other rows 0 for not a criterion


## usda_texture_arsi_hawassa09.csv {#texture-link}
