...
df_combined <- full_join(df_all_sef, df_ba) |> relocate(criterion, .after = crit_code) |> relocate(conc_level_1:conc_level_3, .after = prop_level_3)
...
# output the combined requirements table
  ...
  df_inn <- df_inn |> add_row(crop_name = crop_name_inn,
                              ideotype = ideotype_inn,
                              scenario = scenario_inn) |> select(crop_name, ideotype, scenario) |> arrange(crop_name, ideotype, scenario)
...
df_links_combined <- full_join(df_links_sef, df_links_ba)
