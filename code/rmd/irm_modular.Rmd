...
phen_tmp <- list()
for (i in seq_len(nrow(df_growth_stages) - 2)) {
...
if (nrow(df_criteria_ba3b_m) > 0) {
    for (i in seq_len(nrow(df_growth_stages_req_temp))) {
...
```{r i1_statistics_adop_faolimits_01, echo=FALSE, results='asis', eval=adop_crit}
...
<div class="fold o"> 
```{r i1_statistics_adop_faolimits_02, cache = TRUE, cache.whatever=params$Agg,  out.width="100%", warning=FALSE, eval=adop_crit}
# create subdiv level statistics
# 03/03/2019 A FARROW adde
...
```{r i2_statistics_adop_faolimits_01, echo=FALSE, results='asis', eval=adop_crit}
...
<div class="fold o"> 
```{r i2_statistics_adop_faolimits_02, cache = TRUE, cache.whatever=params$Agg,  out.width="100%", warning=FALSE, eval=adop_crit}
# create subdiv level statistics
# 03/03/2019 A FARROW adde
...
<div class="fold o">
```{r classified compare_score_maps2, cache = TRUE, cache.whatever=params$Agg, out.width="100%", message=FALSE, warning=FALSE, timeit = TRUE, eval=ba_crit}
...
vect_ba_se <-
  mutate(df_ba_se, ba_se_o := eval(expr_ba_se), .keep = c("all"))  %>% vect(geom = c("x", "y")) # evaluate the product sum and convert to vect
...
vect_filename <-
  as.character(paste0("1class_ba_max_", params$INN1, "_", params$INN2, ".shp"))
...
<div class="fold o"> 
```{r classified compare_score_maps3, cache = TRUE, cache.whatever=params$Agg, out.width="100%", message=FALSE, warning=FALSE, timeit = TRUE, eval=ba_crit}
...
We then need to compare the limitations. We can get the limitation value from the raster that has the highest biophysical aptitude optimality value from the two innovations.


<div class="fold o"> 
```{r classified compare_score_maps4, cache = TRUE, cache.whatever=params$Agg, out.width="100%", message=FALSE, warning=FALSE, timeit = TRUE, eval=ba_crit}
...
df_irm_compare_ba_faolimits <-
  cbind(
    select(df_irm_ba_faolimits_rename, x, y, irm_FAO_limit2),
    select(df_irm_i2_ba_faolimits_rename, irm_i2_FAO_limit2)
  )

vect_irm_compare_ba_faolimits <-
  df_irm_compare_ba_faolimits %>% vect(geom = c("x", "y")) # evaluate the product sum and convert to vect

vect_irm_compare_ba_faolimits <- merge(x = vect_irm_compare_ba_faolimits, y = df_irm_faolimits_code, by.x = "irm_FAO_limit2", by.y = "FAO_code_char", all.x = TRUE)
tail(vect_irm_compare_ba_faolimits)

vect_irm_compare_ba_faolimits <- merge(x = vect_irm_compare_ba_faolimits, y = df_irm_i2_faolimits_code, by.x = "irm_i2_FAO_limit2", by.y = "FAO_code_char", all.x = TRUE)
tail(vect_irm_compare_ba_faolimits)


rast_irm_compare_ba_faolimits_1 <-
  rasterize(vect_irm_compare_ba_faolimits, rast_mask_proj, field = "FAO_code_num.x")

rast_irm_compare_ba_faolimits_2 <-
  rasterize(vect_irm_compare_ba_faolimits, rast_mask_proj, field = "FAO_code_num.y")

rast_ba_opt <- rast(list(rast_ba_se, rast_ba_2class_o))

rast_ba_opt_max <-   which.max(rast_ba_opt) # compare with ba optimality of second innovation and choose max value

rast_ba_opt_max_compare <- rast(list(rast_ba_opt_max, rast_irm_compare_ba_faolimits_1, rast_irm_compare_ba_faolimits_2))

plot(rast_ba_opt_max_compare)
#rast_ba_opt_max_compare$max_FAO_limit2 <- ifel(rast_ba_opt_max_compare$rast_ba_opt_max == 1, rast_ba_opt_max_compare$irm_FAO_limit2, rast_ba_opt_max_compare$irm_i2_FAO_limit2)

rast_irm_compare_ba_faolimits <- ifel(rast_ba_opt_max == 1, rast_irm_compare_ba_faolimits_1, rast_irm_compare_ba_faolimits_2)
...
<div class="fold o"> 
```{r classified compare_score_maps5, cache = TRUE, cache.whatever=params$Agg, out.width="100%", message=FALSE, warning=FALSE, message  = FALSE, timeit = TRUE, eval=ba_crit}
# create classified suitability map and validation
# 01/03/2019 A FARROW this chunk split because map not appearing
