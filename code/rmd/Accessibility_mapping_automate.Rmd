...
author: "Atkilt Girma, Amanuel Zenebe, Andy Farrow, Dennis Walvoort and Herman Agricola"
date: "`r Sys.Date()`"
output:
  html_document:  
    code_folding: "hide"
...
# ZONVAR this is the zonal variable within the EXT dataset

# ZONCODE defines the extent of the accessibility model - this is a zonal code within the EXT dataset

# DIVVAR this is the division variable within the EXT dataset

# DIVCODE defines the extent of the model outputs - this is a division code within the EXT dataset
...
Thus, herebelow, we demonstrate on how to compute spatial accessibility. The outputs of these will be used directly during IRM. The  script will output the following:

1. "--_tran_intv.tif"  Access using transport to Kebele centres (FTC)
2. "--_tran_inthv.tif" or "--_tran_zone.tif" Access using transport to Farmers union / zone center/
3. "--_tran_intwv.tif" Access using transport to woreda centers
4. "--_tran_market.tif" acces using transport to market centers

5. "--_walk_intv.tif"  Access by walking to Kebele centres (FTC)
6. "--_walk_inthv.tif" or "--_walk_zone.tif" Access by walking to Farmer Union /zone center/
7. "--_walk_intwv.tif" Access by walking to woreda centers
8. "--_walk_market.tif" access using walking to market centers

NB: "-- is for a two digit character for the study woreda eg. "en" for endamekhoni


```{r ins_figure1, echo=FALSE, fig.cap="Schematic workflow of the accessibility modelling", cache = TRUE, warning=FALSE, out.width= "100%"}
...
vect_ext <- load_vector_data(paste0("subdiv_", params$EXT))

# select the location to be modelled
vect_subdiv <- select_location(vect_ext, params$DIVVAR, params$DIVCODE)

# select the zone to be modelled
vect_div <- select_location(vect_ext, params$ZONVAR, params$ZONCODE)
...
vect_FTC <- load_vector_data(paste0("FTC_", params$DIVCODE))

# load the woreda locations
vect_woredaC <- load_vector_data(paste0("woredaC_", params$DIVCODE))

# load the zonal locations
vect_zoneC <- load_vector_data(paste0("zoneC_", params$ZONCODE))


# load the local market locations
vect_localmarket <- load_vector_data(paste0("localmarket_", params$DIVCODE))

# load the woreda market locations
vect_woredamarket <- load_vector_data(paste0("woredamarket_", params$DIVCODE))

# load the zone market locations
vect_zonemarket <- load_vector_data(paste0("zonemarket_", params$ZONCODE))
...
# load the slope data
rast_slope <- load_raster_data("slope_srtm", rast_slope)
...
rast_landcover <- load_raster_data("globcover", rast_landcover)
...
```{r mask, cache = TRUE, warning=FALSE, out.width=1000}
...
rast_landcover_zone_lea <- terra::project(rast_landcover_zone, wkt_lea, method = 'near')
plot(vect_div_lea_extent)
plot(vect_subdiv_lea, add=T)
...
rast_mask <- rast(extent = vect_div_lea_extent, resolution = params$MASK, vals = c(1))
plot(vect_div_lea_extent)
plot(vect_subdiv_lea, add=T)
...
rm(vect_div,vect_div_extent, vect_FTC, vect_localmarket, vect_roads_ext, vect_roads_zone, vect_woredaC, vect_subdiv, vect_zoneC, vect_zonemarket, rast_slope_zone, vect_woredamarket, rast_landcover_zone)
...
# #col = "transparent"
# 
# st_write(lulc_lea,here::here("output/lulc_lea.shp"), delete_dsn=TRUE)
...
```{r resample, cache = TRUE, warning=FALSE, out.width=500}
...
rast_landcover_zone_lea_rsmp <- terra::resample(rast_landcover_zone_lea, rast_mask, method = "near")
...
# if max speed is 30kmh or below - then assume 30km/hr
# if max speed is between 50kmh and 80kmh - then use the actual speed
# if max speed is above 80kmh - then assume 80km/hr
...
rast_landcover_zone_lea_rsmp_rcl_time <- ((1 / ((rast_landcover_zone_lea_rsmp_rcl * 1000) / 3600)) * params$MASK)
...
writeRaster(rast_cost_v_FTC, here::here(paste0("spatial_data/input/rast_cost_v_FTC_", params$DIVCODE,".tif")), overwrite=T)
...
writeRaster(rast_cost_v_FTC_hr, here::here(paste0("spatial_data/input/rast_cost_v_FTC_hr_", params$DIVCODE,".tif")), overwrite=T)
...
writeRaster(rast_cost_v_woredaC, here::here(paste0("spatial_data/input/rast_cost_v_woredaC_", params$DIVCODE,".tif")), overwrite=T)
...
writeRaster(rast_cost_v_woredaC_hr, here::here(paste0("spatial_data/input/rast_cost_v_woredaC_hr_", params$DIVCODE,".tif")), overwrite=T)
...
writeRaster(rast_cost_v_zoneC, here::here(paste0("spatial_data/input/rast_cost_v_zoneC_", params$DIVCODE,".tif")), overwrite=T)
...
writeRaster(rast_cost_v_zoneC_hr, here::here(paste0("spatial_data/input/rast_cost_v_zoneC_hr_", params$DIVCODE,".tif")), overwrite=T)
...
writeRaster(rast_cost_w_FTC, here::here(paste0("spatial_data/input/rast_cost_w_FTC_", params$DIVCODE,".tif")), overwrite=T)
...
writeRaster(rast_cost_w_FTC_hr, here::here(paste0("spatial_data/input/rast_cost_w_FTC_hr_", params$DIVCODE,".tif")), overwrite=T)
...
writeRaster(rast_cost_w_woredaC, here::here(paste0("spatial_data/input/rast_cost_w_woredaC_", params$DIVCODE,".tif")), overwrite=T)
...
writeRaster(rast_cost_w_woredaC_hr, here::here(paste0("spatial_data/input/rast_cost_w_woredaC_hr_", params$DIVCODE,".tif")), overwrite=T)
...
writeRaster(rast_cost_w_zoneC, here::here(paste0("spatial_data/input/rast_cost_w_zoneC_", params$DIVCODE,".tif")), overwrite=T)
...
writeRaster(rast_cost_w_zoneC_hr, here::here(paste0("spatial_data/input/rast_cost_w_zoneC_hr_", params$DIVCODE,".tif")), overwrite=T)
...
writeRaster(rast_cost_v_localmarket, here::here(paste0("spatial_data/input/rast_cost_v_localmarket_", params$DIVCODE,".tif")), overwrite=T)
...
writeRaster(rast_cost_v_localmarket_hr, here::here(paste0("spatial_data/input/rast_cost_v_localmarket_hr_", params$DIVCODE,".tif")), overwrite=T)
...
writeRaster(rast_cost_v_woredamarket, here::here(paste0("spatial_data/input/rast_cost_v_woredamarket_", params$DIVCODE,".tif")), overwrite=T)
...
writeRaster(rast_cost_v_woredamarket_hr, here::here(paste0("spatial_data/input/rast_cost_v_woredamarket_hr_", params$DIVCODE,".tif")), overwrite=T)
...
writeRaster(rast_cost_v_zonemarket, here::here(paste0("spatial_data/input/rast_cost_v_zonemarket_", params$DIVCODE,".tif")), overwrite=T)
...
writeRaster(rast_cost_v_zonemarket_hr, here::here(paste0("spatial_data/input/rast_cost_v_zonemarket_hr_", params$DIVCODE,".tif")), overwrite=T)
...
writeRaster(rast_cost_w_localmarket, here::here(paste0("spatial_data/input/rast_cost_w_localmarket_", params$DIVCODE,".tif")), overwrite=T)
...
writeRaster(rast_cost_w_localmarket_hr, here::here(paste0("spatial_data/input/rast_cost_w_localmarket_hr_", params$DIVCODE,".tif")), overwrite=T)
...
writeRaster(rast_cost_w_woredamarket, here::here(paste0("spatial_data/input/rast_cost_w_woredamarket_", params$DIVCODE,".tif")), overwrite=T)
...
writeRaster(rast_cost_w_woredamarket_hr, here::here(paste0("spatial_data/input/rast_cost_w_woredamarket_hr_", params$DIVCODE,".tif")), overwrite=T)
...
writeRaster(rast_cost_w_zonemarket, here::here(paste0("spatial_data/input/rast_cost_w_zonemarket_", params$DIVCODE,".tif")), overwrite=T)
...
writeRaster(rast_cost_w_zonemarket_hr, here::here(paste0("spatial_data/input/rast_cost_w_zonemarket_hr_", params$DIVCODE,".tif")), overwrite=T)
