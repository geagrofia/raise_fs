...
```{r i1_load_criteria}
requirements_filename_i1 <-
  as.character(paste(
    "tab_data/input/requirements_",
    params$INN1,
    ".csv",
    sep = ""
  ))
...
df_requirements_i1 <-
  read.csv(here(requirements_filename_i1), na.strings = c("NA",""))
```
...
invalid_rows <- which((is.na(df_requirements_i1$prop_level_1) | is.na(df_requirements_i1$prop_level_2)) & !is.na(df_requirements_i1$threshold))

## Output the rows where where threshold has a value but prop_level_1 or prop_level_2 is NA (or missing)
if (length(invalid_rows) > 0) {
  stop("Rows where threshold has a value but prop_level_1 or prop_level_2 do not:", invalid_rows)
} else {
  print("All rows with values in threshold also have values in prop_level_1 and prop_level_2")
}

# Find the rows where threshold2 has a value but prop_level_2 or prop_level_3 is NA (or missing)
invalid_rows <- which((is.na(df_requirements_i1$prop_level_3) | is.na(df_requirements_i1$prop_level_2)) & !is.na(df_requirements_i1$threshold2))

## Output the rows where where threshold2 has a value but prop_level_2 or prop_level_3 is NA (or missing)
if (length(invalid_rows) > 0) {
  stop("Rows where threshold2 has a value but prop_level_2 or prop_level_3 do not:", invalid_rows)
} else {
  print("All rows with values in threshold2 also have values in prop_level_2 and prop_level_3")
}
...
invalid_rows <- which(!is.na(df_requirements_i1$threshold2) &
                        ((df_requirements_i1$threshold + (df_requirements_i1$width / 2)) > (df_requirements_i1$threshold2 - (df_requirements_i1$width2 /2))
                        )) 

## Output the rows where where threshold2 has a value and boundary widths overlap)
if (length(invalid_rows) > 0) {
  stop("Rows where boundary widths overlap:", df_requirements_i1[invalid_rows,4])
} else {
  print("No rows with values in threshold2 have overlapping boundary widths")
}
...
df_requirements_i1 <- df_requirements_i1[complete.cases(df_requirements_i1$rulebase_number, df_requirements_i1$criterion), ]


df_requirements_short_i1 <-
  df_requirements_i1 %>% dplyr::select(stack, criterion, weight, threshold, threshold2,	width,	width2)
...
# Convert to tree
tree_plot_i1 <- FromDataFrameNetwork(df_requirements_short_i1, c("weight", "threshold",	"threshold2",	"width",	"width2"
))
...
df_requirements_short_i1 <-
  df_requirements_i1 %>% dplyr::select(rulebase_stack, rulebase_number, weight, threshold, threshold2,	width,	width2)


df_requirements_short_i1$rulebase_stack[is.na(df_requirements_short_i1$rulebase_stack)] <-
  "root2"

# Convert to tree
tree_new_i1 <- FromDataFrameNetwork(df_requirements_short_i1, c("weight", "threshold",	"threshold2",	"width",	"width2"
))
...
vect_ext_i1 <- load_vector_data(paste0("subdiv_", params$EXT))

# select the location to be modelled
vect_subdiv_i1 <- select_location(vect_ext_i1, params$DIVCODE, params$LOCCODE)
...
expr <- paste0("vect_subdiv_pt_i1$", params$SUBDIVNAME)
...
vect_triangulation_i1 <- load_vector_data( paste0("triangulation_", params$LOCCODE,"_", params$INN1))
...
for (i in 1:length(list_rast_clas_FAO_cat))   {
  ...
  for (class_type in FAO) {
    ...
    # Filter the joined data for the current limitation
    filtered_data_i1 <-
      vect_subdiv_extr_i1 %>% select(any_of(class_type))
    #print(filtered_data)
    ...
    subdiv_plot_i1 <- ggplot() +
      geom_spatvector(data = filtered_data_i1, aes(group = params$SUBDIVNAME, fill = eval(parse(text = FAO[j_i1])))) +
      labs(
...
for (i in 1:length(list_rast_limits_max_FAO))   {
  ...
  for (limitation_type_i1 in FAO_limits) {
    ...
    # Filter the joined data for the current limitation
    filtered_data_i1 <-
      vect_subdiv_extr_i1 %>% select(any_of(limitation_type_i1))
    #print(filtered_data)
    if (nrow(filtered_data_i1) == 0) {next} 
    ...
    subdiv_plot_i1 <- ggplot() +
      geom_spatvector(data = filtered_data_i1, aes(group = params$SUBDIVNAME, fill = eval(parse(text = FAO_limits[j_i1])))) +
      labs(
...
for (i in 1:length(list_rast_clas_FAO_cat))   {
  ...
  #    select(ID, ha, any_of(FAO)) %>% mutate(across(any_of(FAO), ~ (.x / ha) * 100)) 
...
for (i in 1:length(list_rast_limits_max_FAO))   {
  ...
  # vect_subdiv_extr_i1 <<- vect_subdiv_extr_i1 %>%
  #    select(ID, ha, any_of(FAO_limits)) %>% mutate(across(any_of(FAO_limits), ~ (.x / ha) * 100))
...
df_yield_rb_1_i1 <- dplyr::select(df_requirements_i1, "rulebase_number","criterion","yield") %>% dplyr::filter(!is.na(yield))
...
df_requirements_i2 <-
  read.csv(here(requirements_filename_i2), na.strings = c("NA"))
```
...
df_requirements_i2 <- df_requirements_i2[complete.cases(df_requirements_i2$rulebase_number, df_requirements_i2$criterion), ]


df_requirements_short_i2 <-
  df_requirements_i2 %>% select(stack, criterion, weight, threshold, threshold2,	width,	width2)


df_requirements_short_i2$stack[is.na(df_requirements_short_i2$stack)] <-
  "root2"

# Convert to tree
tree_plot_i2 <- FromDataFrameNetwork(df_requirements_short_i2, c("weight", "threshold",	"threshold2",	"width",	"width2"
))
...
df_requirements_short_i2 <-
  df_requirements_i2 %>% select(rulebase_stack, rulebase_number, weight, threshold, threshold2,	width,	width2)


df_requirements_short_i2$rulebase_stack[is.na(df_requirements_short_i2$rulebase_stack)] <-
  "root2"

# Convert to tree
tree_new_i2 <- FromDataFrameNetwork(df_requirements_short_i2, c("weight", "threshold",	"threshold2",	"width",	"width2"
))
...
df_raster_data_i2 <- droplevels(distinct(df_requirements_i2,
                                      data_file_prefix,
                                      .keep_all = T)) %>% drop_na(data_file_prefix)
...
df_requirements_clim_i2 <- df_requirements_i2[0,]

for (i in clim_raster_data_prefix_i2) { 
# if any of the clim raster data prefix values are in the priorities 
df_requirements_clim_i2 <-
        rbind(df_requirements_clim_i2, subset(df_requirements_i2, grepl(paste0("\\b",i,"\\b"), df_requirements_i2$data_file_prefix)))
}


n_clim_i2 <- nrow(df_requirements_clim_i2)
cat(paste("\n\nThere are ", n_clim_i2, "Climatic criteria: \n"))

  for (i in 1:n_clim_i2) {
  cat(paste("\n",df_requirements_clim_i2[i, 4]," - "))
  rast_clim_mask_i2 <- get(paste0("rast_",df_requirements_clim_i2[i, 12], "_rsmp_i2"))
  if (paste(df_requirements_clim_i2[i, 25]) == "m") {
...
df_requirements_clim_prec_m_i2 <-
  dplyr::filter(df_requirements_clim_i2, prec_temp == "p" &
                  temp_resolution == "m") # filter precipitation criteria

for (i in seq_len(nrow(df_growth_stages_req_prec_i2))) {
  period_name_i2 <- as.character(df_growth_stages_req_prec_i2[i, 1])
  print(paste("Criterion = ", df_requirements_clim_prec_m_i2[i, 4]))
  #      print(period_name)
  
  for (j in seq_len(nrow(df_requirements_clim_prec_m_i2))) {
    if (sum(df_requirements_clim_prec_m_i2[j, 'phen_stage'] == period_name_i2, na.rm = TRUE) == 1) {
      ...
      assign(
        paste0(
          "rast_",
          df_requirements_clim_prec_m_i2[j, 12],
          "_rsmp_",
          df_growth_stages_req_prec_i2[i, 1], "_i2"
        ),
        sum(get(
          paste0("rast_", df_requirements_clim_prec_m_i2[j, 12], "_rsmp_i2")
        ) * get(
      ...
      df_irm_i2 <-
        dplyr::select(df_irm_i2,-any_of(c(as.character(
          paste0(df_requirements_clim_prec_m_i2[j, 3])
        )))) # remove column if name already exists
      
      df_irm_i2 <-
        mutate(df_irm_i2,!!as.character(paste0(df_requirements_clim_prec_m_i2[j, 3])) := new_col_i2,
...
df_requirements_clim_prec_d_i2 <-
  dplyr::filter(df_requirements_clim_i2, prec_temp == "p" &
                  temp_resolution == "d") # filter precipitation criteria

for (i in seq_len(nrow(df_growth_stages_req_prec_i2))) {
  period_name_i2 <- as.character(df_growth_stages_req_prec_i2[i, 1])
  print(paste("Criterion = ", df_requirements_clim_prec_d_i2[i, 4]))
  # print(paste("i =", i))
  # print(period_name)
  for (j in seq_len(nrow(df_requirements_clim_prec_d_i2))) {
      # print(paste("j =", j))
        if (sum(df_requirements_clim_prec_d_i2[j, 'phen_stage'] == period_name_i2, na.rm = TRUE) == 1) {
      print(paste0("period_name = ", as.character(period_name_i2)))
      ...
      assign(
        paste0(
          "rast_",
          df_requirements_clim_prec_d_i2[j, 12],
          "_rsmp_",
          df_growth_stages_req_prec_i2[i, 1], "_i2"
        ),
        sum(get(
          paste0("rast_", df_requirements_clim_prec_d_i2[j, 12], "_rsmp_i2")
        ) * get(
      ...
      df_irm_i2 <-
        dplyr::select(df_irm_i2, -any_of(c(as.character(
          paste0(df_requirements_clim_prec_d_i2[j, 3])
        )))) # remove column if name already exists
      
      df_irm_i2 <-
        mutate(df_irm_i2,
               !!as.character(paste0(df_requirements_clim_prec_d_i2[j, 3])) := new_col_i2,
               .keep = c("all"))   # add the raster values to the df_irm_i2 tibble, this needs to be dynamic so uses the !! and := operators
...
df_requirements_clim_prec_i2 <-
  rbind(df_requirements_clim_prec_m_i2, df_requirements_clim_prec_d_i2)
...
df_requirements_clim_temp_m_i2 <-
  dplyr::filter(df_requirements_clim_i2, prec_temp == "t" &
                  temp_resolution == "m") # filter temperature criteria

if (nrow(df_requirements_clim_temp_m_i2) > 0) {
  #1
  for (i in seq_len(nrow(df_requirements_clim_temp_m_i2))) {
    #2
    #print(paste("i = ", i))
    print(paste("Criterion = ", df_requirements_clim_temp_m_i2[i, 4]))
    for (j in seq_len(nrow(df_growth_stages_req_temp_i2))) {
      ...
      if (sum(df_requirements_clim_temp_m_i2[i, 'phen_stage'] == period_name_i2, na.rm = TRUE) == 1) {
        #4
        if (df_requirements_clim_temp_m_i2[i, 14] == "mean") {
          #5
          print("mean")
          assign(
            paste0(
              "rast_",
              df_requirements_clim_temp_m_i2[i, 12],
              "_rsmp_",
              df_growth_stages_req_temp_i2[j, 1], "_i2"
            ),
            (sum(get(
              paste0("rast_", df_requirements_clim_temp_m_i2[i, 12], "_rsmp_i2")
            ) * get(
          ...
          #-5 5a
          if (df_requirements_clim_temp_m_i2[i, 14] == "min") {
            #6
            print("min")
            assign(
              paste0(
            ...
            #-6 6a
            if (df_requirements_clim_temp_m_i2[i, 14] == "max") {
              #7
              print("max")
              assign(
                paste0(
        ...
        df_irm_i2 <-
          dplyr::select(df_irm_i2,-any_of(c(as.character(
            paste0(df_requirements_clim_temp_m_i2[i, 3])
          )))) # remove column if name already exists
        
        df_irm_i2 <-
          mutate(df_irm_i2,
                 !!as.character(paste0(df_requirements_clim_temp_m_i2[i, 3])) := new_col_i2,
                 .keep = c("all"))   # add the raster values to the df_irm_i2 tibble, this needs to be dynamic so uses the !! and := operators
      } #-4
    }
  }
...
df_requirements_clim_temp_d_i2 <-
  dplyr::filter(df_requirements_clim_i2, prec_temp == "t" &
                  temp_resolution == "d") # filter temperature criteria

if (nrow(df_requirements_clim_temp_d_i2) > 0) {
  #1
  for (i in seq_len(nrow(df_requirements_clim_temp_d_i2))) {
    #2
    #print(paste("i = ", i))
    print(paste("Criterion = ", df_requirements_clim_temp_d_i2[i, 4]))
    for (j in seq_len(nrow(df_growth_stages_req_temp_i2))) {
      #3
      print(paste("j = ", j))
      period_name_i2 <- as.character(df_growth_stages_req_temp_i2[j, 1])
      print(paste0("d period_name = ", as.character(period_name_i2)))
      if (sum(df_requirements_clim_temp_d_i2[i, 'phen_stage'] == period_name_i2, na.rm = TRUE) == 1) {
        #4
        if (df_requirements_clim_temp_d_i2[i, 14] == "mean") {
          #5
          print("mean")
          assign(
            paste0(
              "rast_",
              df_requirements_clim_temp_d_i2[i, 12],
              "_rsmp_",
              df_growth_stages_req_temp_i2[j, 1], "_i2"
            ),
            (sum(get(
              paste0("rast_", df_requirements_clim_temp_d_i2[i, 12], "_rsmp")
            ) * get(
          ...
          #-5 5a
          if (df_requirements_clim_temp_d_i2[i, 14] == "min") {
            #6
            print("min")
            assign(
              paste0(
            ...
            #-6 6a
            if (df_requirements_clim_temp_d_i2[i, 14] == "max") {
              #7
              print("max")
              assign(
                paste0(
        ...
        df_irm_i2 <-
          dplyr::select(df_irm_i2,-any_of(c(as.character(
            paste0(df_requirements_clim_temp_d_i2[i, 3])
          )))) # remove column if name already exists
        
        df_irm_i2 <-
          mutate(df_irm_i2,
                 !!as.character(paste0(df_requirements_clim_temp_d_i2[i, 3])) := new_col_i2,
                 .keep = c("all"))   # add the raster values to the df_irm_i2 tibble, this needs to be dynamic so uses the !! and := operators
      } #-4
    }
  }
...
df_requirements_clim_temp_i2 <-
  rbind(df_requirements_clim_temp_m_i2, df_requirements_clim_temp_d_i2)
...
if (file.exists(here(soil_texture_filename_i2))) {
  ...
  df_requirements_texture_i2 <-
    dplyr::filter(df_raster_data_soil_i2, texture == 1) # filter temperature criteria
  # reclass using df_soil_texture table
  
  if (nrow(df_requirements_texture_i2) > 0) {
    
    #reclassify the texture raster (ensure that it is integer)
    rast_txt_clas_rsmp_i2 <- get(paste0("rast_", df_requirements_texture_i2[1, 12], "_rsmp_i2")) %>% as.int() %>% classify(as.matrix(
...
df_requirements_single_i2 <- inner_join(df_requirements_i2, select(df_raster_data_single_i2, data_file_prefix), by = "data_file_prefix")

for (i in seq_len(nrow(df_requirements_single_i2))) {
  new_col_i2 <-
    get(paste0("rast_", df_requirements_single_i2[i, 12], "_rsmp_i2")) %>% terra::extract(xy_i2) # extract the raster values using xy points
  
  new_col_i2 <- new_col_i2[[1]]
  
  df_irm_i2 <-
    mutate(df_irm_i2,!!as.character(paste0(df_requirements_single_i2[i, 3])) := new_col_i2, .keep = c("all"))   # add the raster values to the df_irm_i2 tibble, this needs to be dynamic so uses the !! and := operators
...
for (i in 1:length(list_rast_clas_FAO_cat))   {
  ...
  # get the names of the FAO classes
  FAO <- as.vector(unique(df_extr_summary_i2$FAO))
  FAO <- FAO[!is.na(FAO)]
  
  df_extr_summary_i2_pivot <- df_extr_summary_i2 %>% pivot_wider(names_from = FAO, values_from = n)
  
  # join the summary data frame to the original subdivision data spatvector
  vect_subdiv_extr_i2 <<-
    left_join(vect_subdiv_i2, df_extr_summary_i2_pivot, by = 'ID')
  
      # save as geojson and csv files
  output_vect(vect_subdiv_extr_i2, paste0(subdiv_criteria_name_i2, "_FAO_area_", params$INN2)) # save as a geojson file
  output_csv(as_tibble(vect_subdiv_extr_i2), paste0(subdiv_criteria_name_i2, "_FAO_area_", params$INN2)) # save as a csv file 
  ...
  for (class_type in unique(vect_subdiv_extr_i2$FAO)) {
    ...
    # Filter the joined data for the current limitation
    filtered_data_i2 <-
      vect_subdiv_extr_i2 %>% select(any_of(class_type))
    #print(filtered_data)
    ...
    subdiv_plot_i2 <- ggplot() +
      geom_spatvector(data = filtered_data_i2, aes(group = params$SUBDIV2, fill = eval(parse(text = FAO[j_i2])))) +
      labs(
...
# get the names of the FAO limitations
  FAO_limits <- as.vector(unique(df_extr_summary_i2$FAO_limit))
  FAO_limits <- FAO_limits[!is.na(FAO_limits)]
  
  
  # pivot wider to give a column for each limit
  df_extr_summary_i2_pivot <- df_extr_summary_i2 %>% pivot_wider(names_from = FAO_limit, values_from = n)
  ...
  vect_subdiv_extr_i2 <<-
    ...
    # save as geojson and csv files
  output_vect(vect_subdiv_extr_i2, paste0(subdiv_criteria_name_i2, "_FAO_limits_area_", params$INN2)) # save as a geojson file
  output_csv(as_tibble(vect_subdiv_extr_i2), paste0(subdiv_criteria_name_i2, "_FAO_limits_area_", params$INN2)) # save as a csv file 
  
  # loop through each existing limitation type 
  for (limitation_type_i2 in FAO_limits) {
    
    # Get the current iteration number
    j_i2 <- match(limitation_type_i2, FAO_limits)
    ...
    # Filter the joined data for the current limitation
    filtered_data_i2 <-
      vect_subdiv_extr_i2 %>% select(any_of(limitation_type_i2))
    #print(filtered_data)
    if (nrow(filtered_data_i2) == 0) {next} 
    ...
    # Create a ggplot for count value per subdivision for the current limitation type using the palette from above
    subdiv_plot_i2 <- ggplot() +
      geom_spatvector(data = filtered_data_i2, aes(group = params$SUBDIV2, fill = eval(parse(text = FAO_limits[j_i2])))) +
      labs(
...
for (i in 1:length(list_rast_clas_FAO_cat))   {
  ...
  # vect_subdiv_extr_i2 <<- vect_subdiv_extr_i2 %>%
  #    select(ID, ha, any_of(FAO)) %>% mutate(across(any_of(FAO), ~ (.x / ha) * 100)) 
  
  vect_subdiv_extr_i2 <<- vect_subdiv_extr_i2 %>% mutate(across(any_of(FAO), ~ (.x / ha) * 100)) #this includes all columns from vect_subdiv_extr_i1 A Farrow 22/05/2024
  
    # save as geojson and csv files
  output_vect(vect_subdiv_extr_i2, paste0(subdiv_criteria_name_i2, "_FAO_pc_", params$INN2)) # save as a geojson file
  output_csv(as_tibble(vect_subdiv_extr_i2), paste0(subdiv_criteria_name_i2, "_FAO_pc_", params$INN2)) # save as a csv file
...
for (i in 1:length(list_rast_limits_max_FAO))   {
  ...
  vect_subdiv_extr_i2 <<- vect_subdiv_extr_i2 %>% mutate(across(any_of(FAO_limits), ~ (.x / ha) * 100)) #this includes all columns from vect_subdiv_extr_i1 A Farrow 22/05/2024
  
  # save as geojson and csv files
  output_vect(vect_subdiv_extr_i2, paste0(subdiv_criteria_name_i2, "_FAO_limits_pc_", params$INN2)) # save as a geojson file
  output_csv(as_tibble(vect_subdiv_extr_i2), paste0(subdiv_criteria_name_i2, "_FAO_limits_pc_", params$INN2)) # save as a csv file 
...
df_yield_rb_1_i2 <- dplyr::select(df_requirements_i2, "rulebase_number","criterion","yield") %>% dplyr::filter(!is.na(yield))
...
# get the cellsize for each raster
rast_yield_pc_cellsize_i2 <- cellSize(rast_yieldpc_i2, unit = "ha")
...
# save as geojson and csv files
output_vect(vect_subdiv_extr_i2, paste0("yieldpc_", params$INN2)) # save as a geojson file
output_csv(as_tibble(vect_subdiv_extr_i2),
           paste0("yieldpc_", params$INN2)) # save as a csv file
...
output_geotiff(rast_yield_i2, paste0("yield_", params$INN2))
  ...
  # get the cellsize for each raster
  rast_yield_cellsize_i2 <- cellSize(rast_yield_i2, unit = "ha")
  ...
  # save as geojson and csv files
  output_vect(vect_subdiv_extr_i2, paste0("yield_", params$INN2)) # save as a geojson file
  output_csv(as_tibble(vect_subdiv_extr_i2),
             paste0("yield_", params$INN2)) # save as a csv file 
...
df_irm_i2$production <- df_irm_i2$yield *  Stat_factor_ha_i2
...
if (summarise(df_irm_i2, Average = mean(production, na.rm = T)) > 0) {
  ...
  # get the cellsize for each raster
  rast_production_cellsize_i2 <- cellSize(rast_production_i2, unit = "ha")
  ...
  # save as geojson and csv files
  output_vect(vect_subdiv_extr_i2, paste0("production_", params$INN2)) # save as a geojson file
  output_csv(as_tibble(vect_subdiv_extr_i2),
             paste0("production_", params$INN2)) # save as a csv file   
...
for (j in 1:length(rb_i12)) {
  ...
  v_clas_fao <- c(-Inf, 0.25, 5, 0.25, 0.4, 4, 0.4, 0.6, 3, 0.6, 0.85, 2, 0.85, Inf, 1)
  m_clas_fao <- matrix(v_clas_fao, ncol = 3, byrow = TRUE)
  
  assign(
    paste0("rast_FAO_", rb_i12[j], "_bin"),
    classify(get(paste0(
      "rast_FAO_", rb_i12[j], "_max"
    )), m_clas_fao, include.lowest = TRUE))
    
    assign(paste0("rast_FAO_", rb_i12[j], "_cat"), get(paste0(
      "rast_FAO_", rb_i12[j], "_bin"
    )))
    ...
    expr_levels <-
      parse(text = paste0("levels(rast_FAO_", rb_i12[j], "_cat) <- df_clas_fao"))
    ...
    assign(
      paste0("rast_FAO_", rb_i12[j], "_brick"),
      rast(list(get(
        paste0("rast_FAO_", rb_i12[j], "_max")
      ), get(
        paste0("rast_FAO_", rb_i12[j], "_bin")
      ), get(
        paste0("rast_FAO_", rb_i12[j], "_cat")
      ))))
      
      names_brick <- c('optimal', 'bin', 'FAO')
      
      expr_names <-
        parse(text = paste0("names(rast_FAO_", rb_i12[j], "_brick) <- names_brick"))
      
      eval(expr_names)
      
      rast_filename_rb_i12_brick <-
        as.character(
          paste(
            "spatial_data/output/FAO_",
            rb_i12[j],
      ...
      print(rast_filename_rb_i12_brick)
      
      writeRaster(get(paste0(
        "rast_FAO_", rb_i12[j], "_brick"
      )), here(rast_filename_rb_i12_brick), overwrite = TRUE
      )
...
```{r compare_subdiv_area_FAO, results = 'asis', out.width="100%"}
...
# loop through the comparison FAO class rasters created above
for (i in length(rb_i12):1)   {
  # get the name of the criteria from the raster name - this is used in the plotting
  subdiv_criteria_name_i12 <- rb_i12[i]

  cat("\n\n\n")
  cat("###", subdiv_criteria_name_i12, "\n") # add headings
  cat("\n\n\n")
  ...
  # get the raster for which statistics are derived for each limitation
  rast_to_plot_i12 <- rast(here(rast_filename_rb_i12_max), lyrs = 3)
  
  # get the cellsize for each raster
  rast_to_plot_cellsize_i12 <- cellSize(rast_to_plot_i12, unit = "ha")
  
  # extract the raster class to a data frame using the subdivisions
  df_extr_i12 <-
    terra::extract(rast_to_plot_i12, vect_subdiv_i1, na.rm = TRUE, ID = T, weights = T)
  
    # extract the raster cellsize to a data frame using the subdivisions
  df_extr_cellsize_i12 <-
    terra::extract(rast_to_plot_cellsize_i12, vect_subdiv_i1, na.rm = TRUE, ID = T, weights = T)
  
  df_extr_i12 <- mutate(df_extr_i12, df_extr_cellsize_i12)
  
   # summarise the class for each subdivision raster ID
  df_extr_summary_i12 <- df_extr_i12 %>%
    group_by(ID) %>%
    count(FAO, wt = (area * weight), .drop = FALSE)
  
  # get the names of the FAO limitations
  FAO <- as.vector(unique(df_extr_summary_i12$FAO))
  FAO <- FAO[!is.na(FAO)]
    
  df_extr_summary_i12_pivot <- df_extr_summary_i12 %>% pivot_wider(names_from = FAO, values_from = n)
  
  # join the summary data frame to the original subdivision data spatvector
  vect_subdiv_extr_i12 <<-
    left_join(vect_subdiv_i1, df_extr_summary_i12_pivot, by = 'ID')
  
  # save as geojson and csv files
  output_vect(
    vect_subdiv_extr_i12,
    paste0(
      subdiv_criteria_name_i12,
      "_FAO_area_",
      params$INN1,
      "_",
      params$INN2
  ...
  output_csv(
    as_tibble(vect_subdiv_extr_i12),
    paste0(
      subdiv_criteria_name_i12,
      "_FAO_area_",
      params$INN1,
  ...
  for (class_type in FAO) {
    ...
    # Get the current iteration number
    j_i12 <- match(class_type, FAO)

    # get the palette using the iteration number
    colour_index_i12 <- j_i12
    colour_index_mod_i12 <- (colour_index_i12 %% 18)
    colour_index_mod_i12 <-
      ifelse(colour_index_mod_i12 == 0, 18, colour_index_mod_i12)
    result_i12 <- palettes_i12[colour_index_mod_i12]
    
    # Filter the joined data for the current limitation
    filtered_data_i12 <-
      vect_subdiv_extr_i12 %>% select(any_of(class_type))
    #print(filtered_data)
    
    if (nrow(filtered_data_i12) == 0) {next} 

  
    # Create a ggplot for count value per subdivision for the current class type using the palette from above
    subdiv_plot_i12 <- ggplot() +
      geom_spatvector(data = filtered_data_i12, aes(group = params$SUBDIVNAME, fill = eval(parse(text = FAO[j_i12])))) +
      labs(
        title = paste(subdiv_criteria_name_i12, "\nClass = ", class_type),
        fill = "Area (ha)"
      ) +
      scale_fill_distiller(
        type = "seq",
...
NEW VERSION
```{r compare_subdiv_area_FAOlimits, results = 'asis', out.width="100%"}
...
# loop through the comparison FAO limitation rasters created above
for (i in length(rb_i12):1)   {
  # get the name of the criteria from the raster name - this is used in the plotting
  subdiv_criteria_name_i12 <- rb_i12[i]
  ...
  rast_filename_rb_i12_max_limits <-
    as.character(
      paste(
        "spatial_data/output/FAO_limits_",
        rb_i12[i],
  ...
  print(rast_filename_rb_i12_max_limits)
  # get the raster for which statistics are derived for each limitation
  rast_to_plot_i12 <- rast(here(rast_filename_rb_i12_max_limits), lyrs = 1)
...
# get the names of the FAO limitations
  FAO_limits <- as.vector(unique(df_extr_summary_i12$FAO_limit))
    FAO_limits <- FAO_limits[!is.na(FAO_limits)]
  
  # pivot wider to give a column for each limit
  df_extr_summary_i12_pivot <- df_extr_summary_i12 %>% pivot_wider(names_from = FAO_limit, values_from = n)
  ...
  # join the summary data frame to the subdivision area data frame
  vect_subdiv_extr_i12 <<-
    left_join(vect_subdiv_extr_i12, df_subdiv_area_i1, by = 'ID')
  ...
  # loop through each existing limitation type 
  for (limitation_type_i12 in FAO_limits) {
    
    # Get the current iteration number
    j_i12 <- match(limitation_type_i12, FAO_limits)
    ...
    # Filter the joined data for the current limitation
    filtered_data_i12 <-
      vect_subdiv_extr_i12 %>% select(any_of(limitation_type_i12))
    #print(filtered_data)
    if (nrow(filtered_data_i12) == 0) {next} 
    ...
    # Create a ggplot for count value per subdivision for the current limitation type using the palette from above
    subdiv_plot_i12 <- ggplot() +
      geom_spatvector(data = filtered_data_i12, aes(group = params$SUBDIVNAME, fill = eval(parse(text = FAO_limits[j_i12])))) +
      labs(
        title = paste(subdiv_criteria_name_i12, "\nLimit = ", limitation_type_i12),
        fill = "Area (ha)"
      ) +
      scale_fill_distiller(
        type = "seq",
...
# for (i in 1:length(list_rast_limits_max_FAO))   {
#   # get the name of the criteria from the raster name - this is used in the plotting
#   subdiv_criteria_name_i12 <- list_rast_limits_max_FAO[i]
#   subdiv_criteria_name_i12 <-
#     sub(paste0(".*", "FAO_"), "", subdiv_criteria_name_i12)
#   
#   cat("\n\n\n")
#   cat("###", subdiv_criteria_name_i12, "\n") # add headings
#   cat("\n\n\n")
#   
#   # get the raster for which statistics are derived for each limitation
#   rast_to_plot_i12 <- get(paste0(list_rast_limits_max_FAO[i]))
#   
#   # get the cellsize for each raster
#   rast_to_plot_cellsize_i12 <- cellSize(rast_to_plot_i12, unit = "ha")
#   
...
#     colour_index_mod_i12 <-
#       ifelse(colour_index_mod_i12 == 0, 18, colour_index_mod_i12)
#     result_i12 <- palettes_i2[colour_index_mod_i12]
#     
#     # Filter the joined data for the current limitation
#     filtered_data_i12 <-
#       vect_subdiv_extr_i12 %>% select(any_of(limitation_type_i12))
...
```{r compare_subdiv_pc_FAO, results = 'asis', out.width="100%"}
...
for (i in length(rb_i12):1)   {
  ...
  vect_subdiv_extr_i12 <<-
   ...
   # join the summary data frame to the subdivision area data frame
  vect_subdiv_extr_i12 <<-
    left_join(vect_subdiv_extr_i12, df_subdiv_area_i2, by = 'ID') 
  
  # vect_subdiv_extr_i12 <<- vect_subdiv_extr_i12 %>%
  #    select(ID, ha, any_of(FAO)) %>% mutate(across(any_of(FAO), ~ (.x / ha) * 100)) 
 
  vect_subdiv_extr_i12 <<- vect_subdiv_extr_i12 %>% mutate(across(any_of(FAO), ~ (.x / ha) * 100)) #this includes all columns from vect_subdiv_extr_i1 A Farrow 22/05/2024
...
```{r compare_subdiv_pc_FAOlimits, results = 'asis', out.width="100%"}
...
# get the names of the FAO limitations
  ...
  vect_subdiv_extr_i12 <<- vect_subdiv_extr_i12 %>% mutate(across(any_of(FAO_limits), ~ (.x / ha) * 100)) #this includes all columns from vect_subdiv_extr_i1 A Farrow 22/05/2024
...
## Yield proportion comparison

<div class="fold o"> 
```{r compare_yieldpc, cache = TRUE, cache.whatever=params$Agg,  out.width="100%", message=FALSE, warning=FALSE, timeit = TRUE}

# yield as percentage of YW potential/water limited/nutrient limited
df_yield_pc_i1 <-
  df_irm_i1 %>% dplyr::select(yieldpc, x , y) %>% na.omit %>% #vect(geom = c("x", "y"))
  dplyr::rename(yieldpc_i1 = yieldpc)
...
df_yield_pc_i12 <- cbind(df_yield_pc_i1, df_yield_pc_i2)
...
df_yield_pc_i12$diff <-
  df_yield_pc_i12$yieldpc_i1 - df_yield_pc_i12$yieldpc_i2
...
df_yield_pc_i12$diff_dec <- df_yield_pc_i12$diff / 100

# max yield pc (when either innovation can be chosen)
df_yield_pc_i12 <- df_yield_pc_i12 %>% rowwise() %>% mutate(rendement = max(c(yieldpc_i1, yieldpc_i2)))

# max innovation name
df_yield_pc_i12$max_inn_name <-
  ifelse(
    df_yield_pc_i12$yieldpc_i1 > df_yield_pc_i12$yieldpc_i2,
    paste(params$INN1),
    ifelse(
...
df_yield_pc_i12$max_inn_id <- as.integer(ifelse(
  df_yield_pc_i12$yieldpc_i1 > df_yield_pc_i12$yieldpc_i2,
  1,
  ifelse(df_yield_pc_i12$yieldpc_i1 < df_yield_pc_i12$yieldpc_i2, 2 , 0)
))

vect_yield_pc_i12 <-
  df_yield_pc_i12 %>% na.omit %>% vect(geom = c("x", "y"))

rast_yield_pc_i12 <-
  rasterize(vect_yield_pc_i12,
            rast_mask_proj_i1,
            field = c("diff", "diff_dec", "rendement", "max_inn_id"))

rast_yield_pc_i12_cat <- rast_yield_pc_i12
df_int <-
  data.frame(id = 0:2,
             max_inn_id = c("equal", params$INN1, params$INN2))
levels(rast_yield_pc_i12_cat$max_inn_id) <- df_int

names(rast_yield_pc_i12_cat) <- c('difference', 'decimal difference', 'rendement' , 'innovation')

# get the cellsize for each raster
rast_yield_pc_cellsize_i12 <- cellSize(rast_yield_pc_i12_cat, unit = "ha")
  
# export
output_geotiff(rast_yield_pc_i12_cat, paste0("yieldpc_difference_", params$INN1,"_", params$INN2))

# extract the yield raster to a data frame using the subdivisions
df_extr_i12 <-
  terra::extract(
    rast_yield_pc_i12_cat$rendement,
    vect_subdiv_i1,
    na.rm = TRUE,
    ID = T,
    weights = T
...
df_extr_summary_i12 <- df_extr_i12 %>%
  group_by(ID) %>%
  summarise(rendement = weighted.mean(rendement, (area * weight), na.rm = TRUE))

# join the summary data frame to the original subdivision data spatvector
vect_subdiv_extr_i12 <<-
  left_join(vect_subdiv_i1, df_extr_summary_i12, by = 'ID')
...
# save as geojson and csv files
output_vect(vect_subdiv_extr_i12,
            paste0("yieldpc_diff_", params$INN1, "_", params$INN2)) # save as a geojson file
output_csv(
  as_tibble(vect_subdiv_extr_i12),
  paste0("yieldpc_diff_", params$INN1, "_", params$INN2)
) # save as a csv file 
  

colour_breaks <- c(-100, 0, 100)
colours <- c("#004D40", "#FFC107", "#D81B60")

gc <- ggplot() +
  geom_spatraster(data = rast_yield_pc_i12_cat,
                  na.rm = TRUE,
                  aes(fill = difference, alpha = after_stat(abs(value)))) +
...
<div class="fold o">
```{r compl_yieldpc, cache = TRUE, cache.whatever=params$Agg, out.width="100%", message=FALSE, warning=FALSE, timeit = TRUE}

if (params$SYS != "comparison") {
      
#complementary score
df_yield_pc_i12$compl <-
  (df_yield_pc_i12$yieldpc_i1 + df_yield_pc_i12$yieldpc_i2)
...
# export
output_geotiff(rast_yield_pc_compl, paste0("yieldpc_complementary_", params$INN1,"_", params$INN2))
        

# extract the yield raster to a data frame using the subdivisions
df_extr_i12 <-
  terra::extract(
    rast_yield_pc_compl$complementary ,
    vect_subdiv_i1,
    na.rm = TRUE,
    ID = T,
    weights = T
...
df_extr_summary_i12 <- df_extr_i12 %>%
  group_by(ID) %>%
  summarise(complementary = weighted.mean(complementary, (area * weight)))
...
# save as geojson and csv files
output_vect(vect_subdiv_extr_i12,
            paste0("yieldpc_compl_", params$INN1, "_", params$INN2)) # save as a geojson file
output_csv(
  as_tibble(vect_subdiv_extr_i12),
  paste0("yieldpc_compl_", params$INN1, "_", params$INN2)
) # save as a csv file 
...
<div class="fold o"> 
```{r compare_yield, cache = TRUE, cache.whatever=params$Agg,  out.width="100%", message=FALSE, warning=FALSE, timeit = TRUE}


# yield as YW potential/water limited/nutrient limited or YI when intercrop
df_yield_i1 <-
  df_irm_i1 %>% dplyr::select(x , y, yield) %>% na.omit %>% #vect(geom = c("x", "y"))
  dplyr::rename(yield_i1 = yield)

df_yield_i2 <-
  df_irm_i2 %>% dplyr::select(yield) %>% na.omit %>%
  dplyr::rename(yield_i2 = yield)

df_yield_i12 <- cbind(df_yield_i1, df_yield_i2)

# relative difference score
df_yield_i12$diff <-
  df_yield_i12$yield_i1 - df_yield_i12$yield_i2

# max yield (when either innovation can be chosen)
df_yield_i12 <- df_yield_i12 %>% rowwise() %>% mutate(max = max(c(yield_i1, yield_i2)))


# max innovation name
df_yield_i12$max_inn_name <-
  ifelse(
    df_yield_i12$yield_i1 > df_yield_i12$yield_i2,
    paste(params$INN1),
    ifelse(
...
# max innovation id for plotting
df_yield_i12$max_inn_id <- as.integer(ifelse(
  df_yield_i12$yield_i1 > df_yield_i12$yield_i2,
  1,
  ifelse(df_yield_i12$yield_i1 < df_yield_i12$yield_i2, 2 , 0)
))

vect_yield_i12 <-
  df_yield_i12 %>% na.omit %>% vect(geom = c("x", "y"))

rast_yield_i12 <-
  rasterize(vect_yield_i12,
            rast_mask_proj_i1,
            field = c("diff", "max", "max_inn_id"))

rast_yield_i12_cat <- rast_yield_i12
df_int <-
  data.frame(id = 0:2,
             max_inn_id = c("equal", params$INN1, params$INN2))
levels(rast_yield_i12_cat$max_inn_id) <- df_int

names(rast_yield_i12_cat) <- c('difference', 'max_yield', 'innovation')

# get the cellsize for each raster
rast_yield_cellsize_i12 <- cellSize(rast_yield_i12_cat, unit = "ha")
  
# export
output_geotiff(rast_yield_i12_cat, paste0("yield_difference_", params$INN1,"_", params$INN2))
...
df_extr_summary_i12 <- df_extr_i12 %>%
  group_by(ID) %>%
  summarise(difference = weighted.mean(difference, (area * weight), na.rm = TRUE))

# summarise the yield for each subdivision raster ID
df_extr_summary_i12a <- df_extr_i12 %>%
  group_by(ID) %>%
  summarise(max_yield = weighted.mean(max_yield, (area * weight), na.rm = TRUE))

df_extr_summary_i12$max_yield <- df_extr_summary_i12a$max_yield
...
# save as geojson and csv files
output_vect(vect_subdiv_extr_i12,
            paste0("yield_diff_", params$INN1, "_", params$INN2)) # save as a geojson file
output_csv(
  as_tibble(vect_subdiv_extr_i12),
  paste0("yield_diff_", params$INN1, "_", params$INN2)
) # save as a csv file 
  
max_yield_i12 <- max(df_yield_values_i2$YW, df_yield_values_i2$YI, df_yield_values_i2$YW, df_yield_values_i2$YI, na.rm = T)

colour_breaks <- c(-max_yield_i12, 0, max_yield_i12)
colours <- c("#004D40", "#FFC107", "#D81B60")

gc <- ggplot() +
  geom_spatraster(data = rast_yield_i12,
                  na.rm = TRUE,
                  aes(fill = diff, alpha = after_stat(abs(value)))) +
  scale_fill_gradientn(
    name = "most suitable\ninnovation (yield)",
    na.value = "transparent",
    limits  = c(-max_yield_i12, max_yield_i12),
    colours = colours[c(1, seq_along(colours), length(colours))],
    breaks = c(-max_yield_i12, 0, max_yield_i12),
    labels = c(paste(params$INN2), "equal", paste(params$INN1))
  ) +
  scale_alpha(guide = 'none')
...
<div class="fold o">
```{r compl_yield, cache = TRUE, cache.whatever=params$Agg, out.width="100%", message=FALSE, warning=FALSE, timeit = TRUE}
...
names(rast_yield_compl) <- c('complementary')

# export
output_geotiff(rast_yield_compl, paste0("yield_complementary_", params$INN1,"_", params$INN2))
        

# extract the yield raster to a data frame using the subdivisions
df_extr_i12 <-
  terra::extract(
    rast_yield_compl$complementary ,
    vect_subdiv_i1,
    na.rm = TRUE,
    ID = T,
    weights = T
...
# save as geojson and csv files
output_vect(vect_subdiv_extr_i12,
            paste0("yield_compl_", params$INN1, "_", params$INN2)) # save as a geojson file
output_csv(
  as_tibble(vect_subdiv_extr_i12),
  paste0("yield_compl_", params$INN1, "_", params$INN2)
) # save as a csv file 

gcsum <- ggplot() +
  geom_spatraster(data = rast_yield_compl,
                  na.rm = TRUE,
                  aes(fill = complementary)) +
  scale_fill_stepsn(
    na.value = "transparent",
    ...
    colours = hcl.colors(palette = "PiYG", 10, rev = FALSE),
    guide = "legend") +
  labs(fill = "Combined yield kg/ha")
