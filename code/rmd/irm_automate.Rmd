...
params:
  ...
  DIVCODEVAL: 30801
  DIVNAMEVAR: W_NAME
  SUBDIVNAMEVAR: RK_NAME
  INN1: pearl_millet_lu
  RES1: 3
  SOS1: 0
  FAOCLASS1: 3
  LIMITS1: 3
...
for (i in length(rb_i12):1)   {
  ...
  for (class_type in FAO) {
    ...
    subdiv_plot_i12 <- ggplot() +
      geom_spatvector(data = filtered_data_i12, aes(group = params$SUBDIVNAMEVAR, fill = eval(parse(text = FAO[j_i12])))) +
      labs(
...
# get the names of the FAO limitations
  ...
  for (limitation_type_i12 in FAO_limits) {
    ...
    # Create a ggplot for count value per subdivision for the current limitation type using the palette from above
    subdiv_plot_i12 <- ggplot() +
      geom_spatvector(data = filtered_data_i12, aes(group = params$SUBDIVNAMEVAR, fill = eval(parse(text = FAO_limits[j_i12])))) +
      labs(
